...
def delete_suspicious_users(suspicious_users, confirm_delete=False):
    ...
    try:
        ...
        for user in suspicious_users:
            try:
                ...
                if cursor.rowcount > 0:
                    deleted_count += 1
                    print(f"✅ Deleted user ID {user_id}: '{user['first_name']}' '{user['last_name']}'")
                else:
                    print(f"⚠️  User ID {user_id} was not found (may have been already deleted)")
        ...
        print(f"   Users that couldn't be deleted: {len(suspicious_users) - deleted_count}")
...
def clean_user_database():
    ...
    response = input("⚠️  Do you want to DELETE these suspicious users? (yes/no): ").strip().lower()
...
def show_recent_users(limit=20):
    ...
    try:
        ...
        for user_id, first_name, last_name, rfid_code, last_active in users:
            print(f"ID {user_id:3}: '{first_name}' '{last_name}' | RFID: {rfid_code} | Last Active: {last_active}")
