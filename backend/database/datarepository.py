...
class QuestionRepository:
    ...
    @staticmethod
    def get_questions_by_theme_and_difficulty(
        themeId, difficultyLevelId, active_only=False
    ):
    ...
    def update_question_metadata(
        question_id, time_limit=None, think_time=None, points=None
    ):
        sql = "UPDATE questions SET "
        updates = []
        params = []
...
class ThemeRepository:
    ...
    @staticmethod
    def create_theme(
        name: str, description: str = None, logo_url: str = None, is_active: bool = True
    ):
...
class UserRepository:
    ...
    def verify_password(password: str, hashed_password: str, salt: str) -> bool:
        ...
        return bcrypt.checkpw(
            password.encode("utf-8"), (hashed_password + salt).encode("utf-8")
        )
    ...
    def get_all_users() -> List[Dict[str, Any]]:
        ...
        sql = "SELECT id, last_name, first_name, email, rfid_code, userRoleId, is_admin, is_banned, is_permanent_ban, banned_until, ban_reason, soul_points, limb_points, last_active, session_expires_at, updated_by FROM users"
    ...
    def get_user_by_rfid(rfid_code: str) -> Optional[Dict[str, Any]]:
        ...
        sql = "SELECT id, last_name, first_name, password_hash, salt, rfid_code, userRoleId, is_admin, is_banned, is_permanent_ban, banned_until, ban_reason, soul_points, limb_points, last_active, session_expires_at, updated_by FROM users WHERE rfid_code = %s"
        params = [rfid_code]
        return Database.get_one_row(sql, params)
    ...
    @staticmethod
    def update_user_names_by_rfid(
        rfid_code: str, first_name: str, last_name: str
    ) -> bool:
    ...
    def verify_password(password_hash: str, salt: str, provided_password: str) -> bool:
        ...
        try:
            return bcrypt.checkpw(
                provided_password.encode("utf-8"), password_hash.encode("utf-8")
            )
        except ValueError:
    ...
    def authenticate_user(
        ...
        if user:
            if UserRepository.verify_password(
...
class IpAddressRepository:
    ...
    def is_ip_banned(ip_address: str) -> bool:
        ...
        result = Database.get_one_row(sql, params)
        return (
            result and result.get("COUNT(*)", 0) > 0
        )  # Access by column name 'COUNT(*)'
...
class UserIpAddressRepository:
    ...
    def create_user_ip_address_link(
        user_id: int, ip_address_id: int, is_primary: bool = False
    ) -> Optional[int]:
        """
        ...
        existing_link = UserIpAddressRepository.get_user_ip_address_link(
    ...
    @staticmethod
    def get_user_ip_address_link(
        user_id: int, ip_address_id: int
    ...
    def set_primary_ip_address(user_id: int, ip_address_id: int) -> bool:
        ...
        sql_unset_primary = (
            "UPDATE userIpAddresses SET is_primary = FALSE WHERE userId = %s"
        )
    ...
    @staticmethod
    def delete_user_ip_address_link_by_user_and_ip(
        user_id: int, ip_address_id: int
    ) -> bool:
        """
...
class SensorDataRepository:
    ...
    def update_sensor_reading(
        data_id, temperature=None, lightIntensity=None, servoPosition=None
    ):
        updates = []
        params = []
...
class ItemRepository:
    ...
    def get_items_by_cost_range(
        min_cost: int = 0, max_cost: int = None
    ) -> List[Dict[str, Any]]:
...
class PlayerItemRepository:
    ...
    def add_item_to_player(
        user_id: int, item_id: int, quantity: int = 1
    ) -> Optional[int]:
    ...
    def debug_insert_test(
        ...
        try:
            ...
            if not item_check_result or (
                isinstance(item_check_result, list) and len(item_check_result) == 0
            ):
    ...
    def get_recent_audit_logs(limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(result, int):
                logger.error(
                    f"Database.execute_sql returned int ({result}) instead of query results"
                )
                logger.error(
                    "This suggests execute_sql is returning rowcount instead of fetchall() results"
                )
                return []
            ...
            if not isinstance(result, list):
                logger.error(
                    f"Database.execute_sql returned unexpected type: {type(result)}"
                )
                return []
...
class ChatLogRepository:
    ...
    @staticmethod
    def flag_message(
        message_id: int, flagged_by: int, flagged_reason: str = None
    ) -> bool:
        """Flag a chat message."""
        sql = """
            UPDATE chatLog 
    ...
    def get_user_message_count(user_id: int, session_id: int) -> int:
        ...
        sql = (
            "SELECT COUNT(*) as count FROM chatLog WHERE userId = %s AND sessionId = %s"
        )
    ...
    def get_chat_message_by_id(
        message_id: int,
    ) -> Optional[Any]:  # Changed return type to Any or tuple
    ...
    @staticmethod
    def get_messages_by_user_in_session(
        session_id: int, user_id: int, limit: int = 100
    ) -> List[Dict[str, Any]]:
    ...
    def get_chat_statistics(session_id: int) -> Dict[str, Any]:
        ...
        sql_total_messages = (
            "SELECT COUNT(*) as total FROM chatLog WHERE sessionId = %s"
        )
...
class SessionPlayerRepository:
    ...
    def increment_correct_answer(
        session_id: int, user_id: int, points: int = 0
    ) -> bool:
...
class PlayerAnswerRepository:
    ...
    def get_all_player_answers_for_session(session_id):
        sql = (
            "SELECT * FROM playerAnswers WHERE sessionId = %s ORDER BY answered_at ASC"
        )
        params = [session_id]
        return Database.get_rows(sql, params)
    ...
    @staticmethod
    def get_player_answers_for_user_in_session_by_question(
    ...
    def get_answer_count_for_question(session_id: int, question_id: int) -> int:
        ...
        try:
            ...
            logger.error(
                f"Error getting answer count for session {session_id}, question {question_id}: {e}"
            )
            return 0
    ...
    def get_total_points_earned_by_user_in_session(session_id, user_id):
        ...
        return (
            result["total_points"]
            if result and result["total_points"] is not None
            else 0
        )

    # UPDATE operations
    @staticmethod
    def update_player_answer(
        player_answer_id,
        answer_id=None,
        is_correct=None,
        points_earned=None,
    ...
    @staticmethod
    def get_question_with_player_answers(
        question_id: int, sessionId: int
    ) -> Optional[Dict[str, Any]]:
        """
    ...
    @staticmethod
    def get_all_player_answers_for_user_in_session(
    ...
    def get_total_score_for_session(session_id):
        ...
        result = Database.get_one_row(sql, params)
        return (
            result["total_score"] if result and result["total_score"] is not None else 0
        )
    ...
    def get_player_score_for_session(session_id, user_id):
        ...
        return (
            result["player_score"]
            if result and result["player_score"] is not None
            else 0
        )
...
class ArticlesRepository:
    ...
    def get_all_articles(
        active_only: bool = False, include_story_info: bool = False
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    def get_articles_by_story_id(
        story_id: int, active_only: bool = True
    ) -> List[Dict[str, Any]]:
...
class StoriesRepository:
    ...
    @staticmethod
    def create_story(
        name: str, slug: Optional[str] = None, description: Optional[str] = None
    ) -> Optional[int]:
        sql = """
...
class GameSaveRepository:
    ...
    @staticmethod
    def create_save(
        user_id: int, save_data: Dict[str, Any], game_version: str = "1.0.0"
    ) -> Optional[int]:
    ...
    def update_save_data(
        user_id: int, save_data: Dict[str, Any], total_play_time: int = None
    ) -> bool:
    ...
    @staticmethod
    def create_backup(
        user_id: int, save_data: Dict[str, Any], version: str = "1.0.0"
    ) -> Optional[int]:
...
class GameResourcesRepository:
    ...
    @staticmethod
    def add_resources(
        user_id: int, stone: int = 0, gold: int = 0, crystals: int = 0
    ) -> bool:
    ...
    def spend_resources(
        ...
        if (
            current["stone_count"] < stone
            or current["gold_count"] < gold
            or current["magical_crystals"] < crystals
        ):
            return False
...
class GameUpgradesRepository:
    ...
    def get_upgrade_stats() -> Dict[str, Any]:
        ...
        stats["vehicle_popularity"] = sorted(
            vehicle_counts.items(), key=lambda x: x[1], reverse=True
        )
...
class SupportMessageRepository:
    ...
    @staticmethod
    def get_messages_by_user(user_id: int, limit: int = 50):
        sql = """SELECT m.id, m.room_id, m.user_id, m.message_text, m.deleted_at, m.created_at,
                        COALESCE(CONCAT(u.first_name, ' ', u.last_name), 'Unknown') as username,
                        r.name as room_name
                 FROM support_messages m
