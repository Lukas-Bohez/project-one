...
class SNESGamepadController:
    ...
    def check_ydotool(self):
        ...
        try:
            result = subprocess.run(
                ["which", "ydotool"], capture_output=True, timeout=2
            )
        ...
        while self.running:
            ...
            with self.button_lock:
                ...
                if (
                    select_pressed
                    and start_pressed
                    and abs(select_time - start_time) < 0.3
                ):
                    ...
                    if hold_time_select >= 1.0 and hold_time_start >= 1.0:
                        ...
                        mode_text = (
                            "CONTROLLER MODE" if self.controller_mode else "MOUSE MODE"
                        )
                        volume_text = (
                            " (Volume controls paused)"
    ...
    def handle_analog_stick(self, axis_x, axis_y):
        ...
        if (
            axis_x != 0 or axis_y != 0
        ) and current_time - self.last_dpad_move >= self.dpad_repeat_delay:
    ...
    def mouse_click(self, button="left"):
        ...
        try:
            ...
            subprocess.run(
                ["ydotool", "click"] + btn_codes[button].split(), timeout=1.0
            )
            return True
        except Exception as e:
            if self.debug:
    ...
    def run(self):
        ...
        print(
            "  B: Right mouse button (hold for drag/scroll) (disabled in controller/pause mode)"
        )
        ...
        print(
            "  SELECT+START (held 1s): Toggle controller/mouse mode + volume controls"
        )
            ...
            while self.running:
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if self.debug:
                            print(
                                f"Button pressed: Index {event.button} -> {button_name}"
                            )
                        ...
                        if self.debug:
                            print(
                                f"Button released: Index {event.button} -> {button_name}"
                            )
                        ...
                        with self.button_lock:
                            if button_name in self.button_states:
                                hold_time = (
                                    time.time()
                                    - self.button_states[button_name]["press_start"]
                                )
