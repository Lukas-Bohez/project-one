        ...
        while True:
            ...
            try:
                ...
                if not showing_rfid and not servo_active:
                    ...
                    if (
                        uid
                    ):  # RFID reader already handles cooldown and duplicate detection
                        rfid_code = uid
                ...
                if (
                    ...
                    try:
                        ...
                        while time.time() - sweep_start_time < sweep_duration:
                            ...
                            lcd.write_line(
                                1, format_second_row(temp, lux, current_degrees)[:16]
                            )
                ...
                if (
                    not showing_rfid
                    and (current_time - last_refresh) >= REFRESH_INTERVAL
                ):
                    try:
                        temp = temp_sensor.read_temperature()
                        lux = light_sensor()
