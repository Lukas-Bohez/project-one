...
def test_sentle_improvements():
    ...
    try:
        ...
        if tables:
            ...
            count = Database.get_one_row(
                "SELECT COUNT(*) as total FROM sentle_daily_sentences"
            )
    ...
    try:
        ...
        old_sentence = Database.get_one_row("""SELECT id, sentence, reuse_count 
        ...
        if old_sentence:
            print(
                f"  ✓ Found reusable sentence (ID: {old_sentence['id']}, reuse_count: {old_sentence['reuse_count']})"
            )
    ...
    # Assert that no tests failed; pytest will report failures if this assertion fails
    assert (
        tests_failed == 0
    ), f"{tests_failed} subchecks failed; see test output for details"


if __name__ == "__main__":
    try:
        test_sentle_improvements()
