...
from models.models import ErrorNotFound, ThemeInput, ThemeResponse
...
async def create_theme_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to create theme. Please try again later."
        )
...
async def delete_theme_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to delete theme. Please try again later."
        )


# Mass migration endpoint for moving questions between themes
@router.post(
    "/api/v1/themes/{source_theme_id}/migrate-to/{target_theme_id}", tags=["Themes"]
)
