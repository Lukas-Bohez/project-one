...
async def list_community_themes(
    request: Request,
    search: Optional[str] = Query(None, description="Search themes by name/description"),
...
async def create_community_theme(theme_data: CommunityThemeCreate, request: Request):
    ...
    logger.info(f"User {user['id']} created community theme '{theme_data.name}' (id={theme_id})")
    theme = CommunityThemeRepository.get_theme_by_id(theme_id)
    return theme


@router.patch("/themes/{theme_id}", response_model=CommunityThemeResponse)
async def update_community_theme(theme_id: int, theme_data: CommunityThemeUpdate, request: Request):
...
async def submit_theme_for_review(theme_id: int, request: Request):
    ...
    if theme['created_by'] != user['id']:
        raise HTTPException(status_code=403, detail="You can only submit your own themes")
    if theme['status'] != 'draft':
        raise HTTPException(status_code=400, detail=f"Theme is already '{theme['status']}', can only submit drafts")
    
    # Require at least 5 questions
    questions = CommunityQuestionRepository.get_questions_by_theme(theme_id)
    if len(questions) < 5:
        raise HTTPException(status_code=400, detail=f"Need at least 5 questions to submit (currently {len(questions)})")
...
async def play_community_theme(theme_id: int):
    ...
    if theme['status'] != 'approved' and not theme.get('is_public'):
        raise HTTPException(status_code=403, detail="This theme is not yet approved for play")
...
@router.get("/themes/{theme_id}/questions", response_model=List[CommunityQuestionResponse])
...
async def create_question(theme_id: int, question_data: CommunityQuestionCreate, request: Request):
    ...
    if not theme:
        raise HTTPException(status_code=404, detail="Theme not found")
    if theme['created_by'] != user['id'] and not is_admin(user):
        raise HTTPException(status_code=403, detail="You can only add questions to your own themes")
    ...
    for answer in question_data.answers:
        CommunityAnswerRepository.create_answer(q_id, answer.answer_text, answer.is_correct)
...
async def update_question(question_id: int, question_data: CommunityQuestionUpdate, request: Request):
    ...
    if theme['created_by'] != user['id'] and not is_admin(user):
        raise HTTPException(status_code=403, detail="You can only edit questions in your own themes")
...
async def delete_question(question_id: int, request: Request):
    ...
    theme = CommunityThemeRepository.get_theme_by_id(question['community_theme_id'])
    if theme['created_by'] != user['id'] and not is_admin(user):
        raise HTTPException(status_code=403, detail="You can only delete questions in your own themes")
...
async def upload_csv_questions(
    ...
    if not theme:
        raise HTTPException(status_code=404, detail="Theme not found")
    if theme['created_by'] != user['id'] and not is_admin(user):
        raise HTTPException(status_code=403, detail="You can only upload to your own themes")
    ...
    # Import questions
    imported, failed, errors = CommunityQuestionRepository.bulk_create_from_csv(theme_id, rows)
    ...
    logger.info(f"CSV upload by user {user['id']}: {imported} imported, {failed} failed from '{file.filename}'")
...
async def review_theme(theme_id: int, review: ReviewAction, request: Request):
    ...
    if theme['status'] != 'pending_review':
        raise HTTPException(status_code=400, detail=f"Theme is '{theme['status']}', not pending review")
...
async def promote_theme_to_official(theme_id: int, request: Request):
    ...
    if not theme:
        raise HTTPException(status_code=404, detail="Theme not found")
    if theme['status'] != 'approved':
        raise HTTPException(status_code=400, detail="Only approved themes can be promoted")
    ...
    logger.info(f"Admin {user['id']} promoted community theme {theme_id} to official theme {official_id}")
    return {"status": "success", "official_theme_id": official_id, "message": "Theme promoted to official!"}
