...
async def update_employee(employee_id: int, employee_update: EmployeeUpdate):
    ...
    if employee.get("user_id") and user_profile_updates:
        UserEmailRepository.update_user_profile(
            employee["user_id"], user_profile_updates
        )
...
@router.get(
    "/businesses/{business_id}/employees", response_model=List[EmployeeResponse]
)
async def get_business_employees(business_id: int, status: Optional[str] = None):
...
async def create_shift(shift: ShiftCreate):
    ...
    shifts = ManageShiftRepository.get_shifts_by_date_range(
        shift.business_id, shift.shift_date, shift.shift_date
    )
...
async def get_business_shifts(business_id: int, start_date: date, end_date: date):
...
async def get_employee_shifts(employee_id: int, start_date: date, end_date: date):
    """Get shifts for a specific employee"""
    shifts = ManageShiftRepository.get_employee_shifts(
...
@router.get(
    "/businesses/{business_id}/time-off/pending",
...
@router.post(
    "/warnings", response_model=WarningResponse, status_code=status.HTTP_201_CREATED
)
async def create_warning(warning: WarningCreate):
    ...
    created_warnings = ManageWarningRepository.get_employee_warnings(
        warning.employee_id, active_only=False
    )
...
async def get_dashboard_stats(business_id: int):
    ...
    # Get today's shifts
    today = date.today()
    today_shifts = ManageShiftRepository.get_shifts_by_date_range(
    ...
    week_shifts = ManageShiftRepository.get_shifts_by_date_range(
    ...
    for shift in week_shifts:
        if shift["status"] in ["completed", "confirmed"]:
            ...
            employee = next(
                (e for e in all_employees if e["id"] == shift["employee_id"]), None
            )
...
@router.get(
    "/employees/{employee_id}/time-off", response_model=List[TimeOffRequestResponse]
)
...
async def review_time_off_request(request_id: int, review: TimeOffRequestReview):
    ...
    reviewed = ManageTimeOffRepository.review_time_off_request(
        request_id, review.dict()
    )
    ...
    await broadcast_time_off_reviewed(
        time_off_request["business_id"], request_obj, approved
    )
...
async def query_shifts(
    ...
    if not business_id:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="business_id is required"
        )
...
async def update_shift(shift_id: int, update_data: ShiftUpdate):
    ...
    # Update the shift
    success = ManageShiftRepository.update_shift(
        shift_id, update_data.dict(exclude_unset=True)
    )
...
async def websocket_endpoint(websocket: WebSocket, business_id: int, user_id: int):
    ...
    try:
        ...
        logger.info(
            f"WebSocket client {user_id} disconnected from business {business_id}"
        )
