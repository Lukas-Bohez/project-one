...
from database.datarepository import StoriesRepository, ArticlesRepository
...
def get_story_mapping(article_index: int) -> tuple[str, int]:
    ...
    if 1 <= article_num <= 2:
        ...
        return ("Age of Bargaining, Part 2: Tournament of Time", article_num - 79)  # 0-6
    else:
...
def format_article_for_db(article: Dict[str, Any], date_written: str, article_index: int) -> Dict[str, Any]:
    """
...
def main():
    ...
    try:
        ...
        response = input(f"Found {len(articles)} articles. Import them to the database? (y/N): ")
        if response.lower() != 'y':
