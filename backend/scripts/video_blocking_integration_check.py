...
# Skip if the API server is not running locally
try:
    requests.get(f"{BASE_URL}/api/v1/themes/", timeout=1)
except Exception:
    pytest.skip(
        "API server not running on localhost:8000 - skipping video blocking integration tests",
