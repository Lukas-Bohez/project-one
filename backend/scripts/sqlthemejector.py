...
def replace_jack_with_kael():
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM questions WHERE question_text LIKE '%Jack%' OR explanation LIKE '%Jack%'")
        ...
        print(f"Found {questions_before} questions and {answers_before} answers with 'Jack'")
...
def check_kael_mentions():
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM questions WHERE question_text LIKE '%Kael%' OR explanation LIKE '%Kael%'")
