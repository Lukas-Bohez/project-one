...
function unloadFolder() {
  ...
  statusEl.textContent =
    currentMode === 'bundled'
      ? 'Switched to bundled mode'
      : 'Files cleared - Select files or folder';
}
...
function renderAllMode() {
  ...
  if (songs.length > 0) {
    ...
    songsHeader.style.cssText =
      'padding: var(--spacing-sm) 0; font-weight: 600; color: var(--primary-color); font-size: var(--font-size-sm); border-bottom: 2px solid var(--primary-color); margin-bottom: var(--spacing-sm);';
    ...
    songs.forEach((track) => {
      ...
      div.className =
        'song-item audio-item-compact' + (actualIndex === currentIndex ? ' selected' : '');
      div.dataset.index = actualIndex;
      div.style.cssText =
        'display: flex; align-items: center; padding: var(--spacing-xs) var(--spacing-sm); margin-bottom: 4px; border-radius: var(--border-radius-sm); cursor: pointer; transition: all 0.2s; background: var(--bg-tertiary); border: 1px solid transparent; position: relative; padding-left: 24px;';
      ...
      if (track.thumbnail) {
        ...
        thumb.style.cssText =
          'width: 36px; height: 36px; border-radius: 3px; margin-right: 10px; object-fit: cover; flex-shrink: 0;';
        div.appendChild(thumb);
      } else {
        const musicIcon = document.createElement('span');
        musicIcon.textContent = '♪';
        musicIcon.style.cssText =
          'font-size: 20px; margin-right: 10px; color: var(--primary-color); flex-shrink: 0; width: 36px; text-align: center;';
      ...
      titleDiv.style.cssText =
        'font-size: var(--font-size-sm); font-weight: 500; color: var(--text-primary); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;';
  ...
  if (videos.length > 0) {
    ...
    videosHeader.style.cssText =
      'padding: var(--spacing-sm) 0 var(--spacing-xs) 0; font-weight: 600; color: var(--primary-color); font-size: var(--font-size-sm); border-bottom: 2px solid var(--primary-color); margin-top: var(--spacing-md); margin-bottom: var(--spacing-sm);';
    ...
    videos.forEach((track) => {
      ...
      const div = document.createElement('div');
      div.className =
        'song-item video-item-compact' + (actualIndex === currentIndex ? ' selected' : '');
      div.dataset.index = actualIndex;
      div.style.cssText =
        'display: flex; align-items: center; padding: var(--spacing-xs) var(--spacing-sm); margin-bottom: 4px; border-radius: var(--border-radius-sm); cursor: pointer; transition: all 0.2s; background: var(--bg-tertiary); border: 1px solid transparent;';

      // Video thumbnail (compact)
      const thumbWrapper = document.createElement('div');
      thumbWrapper.style.cssText =
        'position: relative; width: 48px; height: 27px; flex-shrink: 0; margin-right: 10px; border-radius: 3px; overflow: hidden; background: var(--bg-30);';
...
function renderSongsMode() {
  ...
  songs.forEach((track) => {
    ...
    if (track.thumbnail) {
      ...
      thumb.style.cssText =
        'width: 48px; height: 48px; border-radius: 4px; margin-right: 12px; object-fit: cover; flex-shrink: 0;';
      div.appendChild(thumb);
    } else {
      const musicIcon = document.createElement('span');
      musicIcon.textContent = '🎵';
      musicIcon.style.cssText =
        'font-size: 32px; margin-right: 12px; flex-shrink: 0; width: 48px; text-align: center;';
      div.appendChild(musicIcon);
    }
...
async function playTrack(index) {
  ...
  try {
    ...
    try {
      ...
      statusEl.textContent =
        '❌ Playback failed: ' + (playErr.message || 'Media format not supported');
      isPlaying = false;
      updateUI();
    }
  } catch (err) {
...
function updateUI() {
  ...
  if (nowPlayingEl) {
    if (tracks.length > 0 && currentIndex >= 0 && currentIndex < tracks.length) {
      ...
      nowPlayingEl.textContent =
        currentMode === 'bundled' ? 'Loading tracks...' : 'Select files or folder';
    }
  }
  ...
  if (statusEl) {
    if (tracks.length > 0 && currentIndex >= 0 && currentIndex < tracks.length) {
      ...
      statusEl.textContent =
        currentMode === 'bundled' ? 'Loading bundled tracks...' : 'Select files or folder';
    }
  }
}
