...
const getDisplayTitle = (song) => {
  ...
  if (typeof song === 'string') {
    ...
    const found = songList.find(
      (s) =>
        s.id === song ||
        s.filename === song ||
        s.title === song ||
        (s.url && s.url.split('/').pop() === song)
    );
  ...
  try {
    ...
    if (volRaw !== null) {
      ...
      if (Number.isFinite(v) && v <= 0.0001) {
        // Only keep cached volume if it's already extremely quiet (<= 0.01%)
...
const activateLofiPlayer = () => {
  ...
  try {
    ...
    console.warn('Could not persist activation:', e);
  }
  ...
  window.addEventListener('beforeunload', () => {
    ...
    Object.values(_fileUrlMap).forEach((u) => {
      try {
  ...
  try {
    debouncedSavePlayerState = debounce(savePlayerState, 2000);
  } catch (_) {}
  ...
  setInterval(() => {
    if (isPlaying && !audioPlayer.paused) {
      try {
        if (debouncedSavePlayerState) debouncedSavePlayerState();
        else savePlayerState();
      } catch (_) {}
    }
  }, 10000);
};
            ...
            if (cached && cached.length > 0) {
              songList = cached.map((entry) => {
                const id =
                  ...
                  entry.filename ||
                  entry.key ||
                  `cached-${Math.random().toString(36).slice(2, 8)}`;
                  ...
                  if (deferredPromptScheduled) {
                    ...
                    const deferredHandler = async () => {
                      ...
                      try {
                        ...
                        if (!prompted) {
                          console.log(
                            '⚠️ User did not provide folder; falling back to manual list'
                          );
                          songList = [...manualSongList];
                        } else {
                          // If folder provided, try to resume
                          const savedId = savedPlaybackState.currentSong;
                          const match = songList.find(
                            (s) =>
                              s.id === savedId ||
                              s.filename === savedId ||
                              (s.url && s.url.split('/').pop() === savedId)
                          );
                          if (match)
                            loadAndPlayCached(
                              match,
                              savedPlaybackState.currentTime || 0,
                              savedPlaybackState.volume || config.volume,
                              !!savedPlaybackState.isPlaying
                            );
                        }
                      } catch (e) {
                        console.warn('Deferred prompt handler failed:', e);
                      }
                    };
...
const handleInitialSongLoad = () => {
  if (config.persistence.enabled && savedPlaybackState && savedPlaybackState.currentSong) {
    ...
    const savedSong = songList.find(
      (song) =>
        song.id === savedSongId ||
        song.filename === savedSongId ||
        (song.url && song.url.split('/').pop() === savedSongId)
    );

    if (savedSong) {
      console.log(
        `Attempting to load saved song: ${getDisplayTitle(savedSong)} at ${savedPlaybackState.currentTime.toFixed(1)}s`
      );
...
const playNextSong = (initiator = 'user') => {
  ...
  if (initiator === 'auto' && config.repeat === 'one' && audioPlayer.src) {
    ...
    try {
      ...
      audioPlayer
        ...
        .then(() => {
          isPlaying = true;
          const currentFilename = audioPlayer.src
            ? audioPlayer.src.split('/').pop().split('?')[0]
            : null;
  ...
  console.log(
    ...
    `(${config.shuffle ? 'shuffle' : 'sequential'} mode)`
  );
...
const handleFailedSong = (songName) => {
  ...
  if (!songRef) {
    ...
    failedObj = songList.find(
      (s) =>
        s.id === songRef ||
        s.filename === songRef ||
        s.title === songRef ||
        (s.url && s.url.split('/').pop() === songRef)
    );
  }
  ...
  const nowPlayingDiv = document.getElementById('now-playing');
  if (nowPlayingDiv)
    nowPlayingDiv.textContent = `Error playing: ${getDisplayTitle(failedObj || songRef)}`;
  ...
  if (failedObj) {
    ...
    songList = songList.filter(
      (s) => !(s.id === songRef || s.title === songRef || s.filename === songRef)
    );
    console.log(
      'Removed failed song by fallback filter, remaining:',
      songList.length,
      `(removed ${before - songList.length})`
    );
  }
...
const updateUIForPlaying = (songRef) => {
  ...
  if (!songObj && typeof songRef === 'string') {
    ...
    songObj = songList.find(
      (s) =>
        s.filename === ref ||
        s.filename === ref + '.mp3' ||
        s.filename === ref + '.wav' ||
        (s.filename && s.filename.split('/').pop() === ref)
    );
    // 2) match by title exact or contains
    if (!songObj)
      songObj = songList.find((s) => s.title === ref || (s.title && s.title.includes(ref)));
    // 3) match by metadata fields
    if (!songObj)
      songObj = songList.find(
        (s) =>
          s.metadata &&
          (s.metadata.title === ref ||
            (s.metadata.title && s.metadata.title.includes(ref)) ||
            s.metadata.videoId === ref)
      );
    ...
    if (!songObj && /^[0-9a-f-]{8,}$/i.test(ref)) {
      songObj = songList.find(
        (s) =>
          s.id === ref ||
          (s.metadata &&
            (s.metadata.videoId === ref || s.metadata.id === ref || s.metadata.key === ref))
      );
    }
  }
  ...
  if (uuidLike.test(String(display))) {
    if (songObj) {
      ...
      else if (songObj.metadata && songObj.metadata.title)
        display = prettyTitle(songObj.metadata.title);
  ...
  if (playPauseBtn) {
    ...
    playPauseBtn.onmouseover = () =>
      (playPauseBtn.style.backgroundColor = 'var(--info-color, #0d61aa)');
...
const updateUIForPaused = (songRef) => {
  ...
  if (playPauseBtn) {
    ...
    playPauseBtn.style.backgroundColor = 'var(--primary-color, #2c4c7c)';
    playPauseBtn.onmouseover = () =>
      (playPauseBtn.style.backgroundColor = 'var(--secondary-color, #0c4061)');
    playPauseBtn.onmouseout = () =>
      (playPauseBtn.style.backgroundColor = 'var(--primary-color, #2c4c7c)');
  }
};
...
const savePlayerState = () => {
  ...
  try {
    if (currentSongId) savedSong = currentSongId;
      ...
      const matched = songList.find(
        (s) =>
          s.url === src ||
          (s.url && src.includes(s.url)) ||
          s.id === src ||
          s.filename === src.split('/').pop()
      );
...
const loadPlayerState = () => {
  ...
  try {
    ...
    if (savedStateString) {
      ...
      config.shuffle =
        savedPlaybackState.shuffle !== undefined ? savedPlaybackState.shuffle : config.shuffle;
      config.repeat =
        savedPlaybackState.repeat !== undefined ? savedPlaybackState.repeat : config.repeat;
...
const getSavedDirectoryHandle = async () => {
  try {
    ...
    console.warn('Failed to read saved directory handle:', e);
    return null;
  }
};
...
const clearSavedDirectoryHandle = async () => {
  try {
    ...
    console.warn('Failed to clear saved handle:', e);
    return false;
  }
};
...
const promptUserToSelectFolder = async () => {
    ...
    if ('showDirectoryPicker' in window) {
      ...
      try {
        localStorage.setItem(
          'lofi_saved_folder_files',
          JSON.stringify(songList.map((s) => s.filename))
        );
      } catch (_) {}
      createShuffledPlaylist();
      populateSongSelector();
  ...
  return new Promise((resolve) => {
    try {
      ...
      tmp.accept = '.mp3,.wav';
      try {
        tmp.setAttribute('webkitdirectory', '');
      } catch (_) {}
    ...
    if (audioPlayer.src && audioPlayer.paused) {
      audioPlayer
        ...
        .then(() => {
          ...
          const nowPlayingDiv = document.getElementById('now-playing');
          if (nowPlayingDiv) {
            const songTitle = getDisplayTitle(
              currentSongId ||
...
console.log(
  'Manual controls: lofi.skip(), lofi.stop(), lofi.volume(0.5), lofi.list(), lofi.clearHistory()'
);
...
const toggleShuffle = () => {
  ...
  if (shuffleBtn) {
    ...
    shuffleBtn.style.backgroundColor = config.shuffle
      ? 'var(--success-color, #2e8b34)'
...
const createLofiModal = () => {
  ...
  nowPlayingDiv.style.color =
    'rgba(var(--light-color-r, 244), var(--light-color-g, 248), var(--light-color-b, 252), 0.8)';
  ...
  // Mode Toggle Button
  const modeToggleBtn = createButton(
    config.mode === 'cache' ? '📁 Local Mode' : '💾 Cache Mode',
    () => {
      ...
      const songSelector = document.getElementById('song-selector');
      if (songSelector)
        songSelector.innerHTML = '<option value="">Switching to ' + newMode + '...</option>';
      ...
      if (newMode !== 'cache') {
        if (folderSelectorDiv.parentNode)
          folderSelectorDiv.parentNode.removeChild(folderSelectorDiv);
        Object.values(_fileUrlMap).forEach((u) => {
          try {
            URL.revokeObjectURL(u);
          } catch (_) {}
  ...
  const rememberBtn = createButton(
    '💾 Remember Folder',
    async () => {
      if (!('showDirectoryPicker' in window)) {
  ...
  const clearRememberedBtn = createButton(
    '🧹 Clear Remembered Folder',
    async () => {
      const ok = confirm(
        'Clear the remembered folder? This will remove the saved directory handle.'
      );
      if (!ok) return;
  ...
  // Clear Folder Button
  const clearFolderBtn = createButton(
    '🗑️ Clear Folder',
    () => {
      if (!selectedFolderFiles || selectedFolderFiles.length === 0) {
  ...
  folderInput.addEventListener('change', (e) => {
    ...
    if (!files || files.length === 0) {
      alert(
        'No audio files found in the selected folder. Please choose a folder with .mp3 or .wav files.'
      );
      return;
    }
  ...
  const disableBtn = createButton(
    ...
    () => {
      if (
        confirm(
          'Are you sure you want to disable music? This will stop playback and hide the player.'
        )
      ) {
        lofi.stop();
  ...
  const logoutClearCacheBtn = createButton(
    'Log out of quiz user account',
    async () => {
      if (
        confirm(
          'Are you sure you want to log out? This will clear ALL cached data and force a complete reload. This may take a moment.'
        )
      ) {
        try {
          // Show loading state
          ...
          alert(
            'An error occurred while clearing caches, but the page will still reload to ensure fresh content.'
          );
  ...
  const handlePlayingEvent = () => {
    ...
    const displayTarget =
      trackedId || (audioPlayer.src ? audioPlayer.src.split('/').pop().split('?')[0] : null);
...
const createButton = (text, onClickHandler, bgColor) => {
  ...
  let baseColorValue = getComputedStyle(document.documentElement).getPropertyValue(
    bgColor.replace('var(', '').replace(')', '').split(',')[0].trim()
  );
