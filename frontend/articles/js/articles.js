...
class ArticlesManager {
  ...
  generateContextualQuery(article) {
    ...
    const conceptMap = {
      'wealth-inequality':
        'economic inequality protest, rich vs poor, financial disparity, social justice',
  ...
  getFilteredAndSortedArticles() {
    ...
    if (this.currentFilter !== 'all') {
      filtered = filtered.filter(
        (article) =>
          article.category === this.currentFilter || article.tags.includes(this.currentFilter)
      );
    }
