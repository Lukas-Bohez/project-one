# Assets

## CSS

- /css/styles.css
- /css/pages/quiz/quiz.css
- /css/pages/quiz/chat.css
- /css/footer.css

## JS

- https://cdn.socket.io/4.7.5/socket.io.min.js
- https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-8418485814964449
- /js/themeManager.js
- /js/pages/quiz/items.js
- /js/infomodal.js
- /js/pages/quiz/gamepadQuiz.js
- /js/pages/quiz/quiz.js
- /js/pages/quiz/quizmain.js
- /js/pages/quiz/quizlogic2.js
- /js/pages/quiz/quizlogic.js
- /js/pages/quiz/quizlogic3.js
- /js/pages/quiz/quizlogic4.js
- /js/ip.js
- /js/pages/quiz/quizPlayerListManager.js
- /js/pages/quiz/quizlogin.js
- /js/pages/quiz/chat.js
- /js/pages/quiz/quizItems.js
- /js/privacy.js
- https://cdnjs.buymeacoffee.com/1.0.0/button.prod.min.js
