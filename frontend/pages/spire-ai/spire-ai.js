...
(function () {
  ...
  function api(endpoint, opts = {}) {
    ...
    if (currentUser) {
      ...
      const storedPassword =
        localStorage.getItem('sai_password') || localStorage.getItem(STORAGE_KEYS.PASSWORD);
      if (storedPassword) {
  ...
  function getStoredUser() {
    ...
    const looksLikePlaceholder =
      String(fn || '').toLowerCase() === 'gamepad' && String(ln || '').toLowerCase() === 'user';
  ...
  async function loadActiveSessions() {
    ...
    try {
      ...
      if (sessions.length === 0) {
        ...
        h3.textContent = 'No active sessions right now';
        const p = document.createElement('p');
        p.textContent = 'Start a new quiz below or wait for someone to create one!';
        noEl.appendChild(icon);
        noEl.appendChild(h3);
        noEl.appendChild(p);
      ...
      sessions.forEach((s) => {
        const phaseLabel =
          { voting: 'Voting', theme_display: 'Theme Reveal', quiz: 'Playing' }[s.phase] ||
          'Waiting';
        const phaseClass =
          { voting: 'voting', theme_display: 'playing', quiz: 'playing' }[s.phase] || 'waiting';

        const card = document.createElement('div');
        card.className = 'sai-session-card';

        const header = document.createElement('div');
        header.className = 'sai-session-card__header';
        const title = document.createElement('span');
        title.className = 'sai-session-card__title';
        title.textContent = typeof esc === 'function' ? esc(s.name) : s.name;
        const live = document.createElement('span');
        live.className = 'sai-session-card__live';
        live.textContent = 'Live';
        header.appendChild(title);
        header.appendChild(live);

        const meta = document.createElement('div');
        meta.className = 'sai-session-card__meta';
        const playersSpan = document.createElement('span');
        const usersIcon = document.createElement('i');
        usersIcon.className = 'fas fa-users';
        playersSpan.appendChild(usersIcon);
        playersSpan.appendChild(
        ...
        if (s.theme_name) {
          ...
          const pal = document.createElement('i');
          pal.className = 'fas fa-palette';
          themeSpan.appendChild(pal);
          themeSpan.appendChild(
            document.createTextNode(
              ' ' + (typeof esc === 'function' ? esc(s.theme_name) : s.theme_name)
            )
          );
        ...
        phaseSpan.className = 'sai-session-card__phase sai-phase--' + phaseClass;
        phaseSpan.textContent = phaseLabel;
        meta.appendChild(phaseSpan);

        const actions = document.createElement('div');
        actions.className = 'sai-session-card__actions';
        const joinBtn = document.createElement('button');
        joinBtn.className = 'sai-btn sai-btn--success sai-btn--sm';
      ...
      icon.className = 'fas fa-satellite-dish';
      const h3 = document.createElement('h3');
      h3.textContent = 'No active sessions';
      const p = document.createElement('p');
      p.textContent = 'Start a new quiz below to begin playing';
      noEl.appendChild(icon);
      noEl.appendChild(h3);
      noEl.appendChild(p);
      container.appendChild(noEl);
  ...
  function setupStartSession() {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        toast(
          `Session created with ${result.theme_count} theme${result.theme_count > 1 ? 's' : ''} - voting starts now!`,
          'success'
        );
  ...
  window.reviewTheme = async function (themeId, action) {
    ...
    try {
      ...
      toast(
        `Theme ${action === 'approve' ? 'approved' : 'rejected'}!`,
  ...
  window.openThemeDetail = async function (themeId) {
    try {
      ...
      let questions = [];
      try {
        questions = await api(`/themes/${themeId}/questions`);
      } catch {}
  ...
  function setupCharCounters() {
    ...
    if (descInput && descCount) {
      const update = () =>
        (descCount.textContent = `${descInput.value.length}/${descInput.maxLength}`);
  ...
  function renderQuestionsList(questions) {
    ...
    if (questions.length === 0) {
      list.innerHTML =
        '<p style="color:var(--sai-text-muted);text-align:center;padding:20px;">No questions yet. Add your first one below!</p>';
      return;
    }
  ...
  function setupQuestionForm() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!answers.some((a) => a.is_correct))
        return toast('Mark at least one correct answer', 'error');
  ...
  function setupCsvUpload() {
    ...
    dropZone.addEventListener('dragover', (e) => {
      e.preventDefault();
      dropZone.classList.add('sai-dropzone--active');
    });
  ...
  function setupPasteText() {
    ...
    parseBtn.addEventListener('click', async () => {
      ...
      toast(
        `Imported ${imported} question${imported !== 1 ? 's' : ''} from pasted text!`,
        'success'
      );
