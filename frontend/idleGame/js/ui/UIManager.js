class UIManager {
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener('click', () => this.switchToRegister());
  ...
  async handleSave() {
    if (!this.isLoggedIn) {
      this.showNotification(
        '❌ Cannot save game - Please login or create an account first!',
        'error'
      );
  ...
  handleReset() {
    if (
      confirm(
        'Are you sure you want to reset your game? This will delete all progress but keep you logged in!'
      )
    ) {
