...
class NewResourceManager {
  ...
  cleanupInventory() {
    ...
    if (this.state.cityInventory?.finished) {
      Object.keys(this.state.cityInventory.finished).forEach((item) => {
        if (
          !validOutputs.has(item) &&
          !this.catalog.processed[item] &&
          !this.catalog.finished[item]
        ) {
  ...
  pickNextForTransport(autoTransportPrefs = null) {
    ...
    if (theme?.crafting) {
      ...
      if (theme.crafting.intermediate)
        craftedItems[theme.crafting.intermediate.result] = 'intermediate';
  ...
  sellOne(item) {
    const meta =
      this.catalog.finished[item] || this.catalog.processed[item] || this.catalog.raw[item];
    if (!meta) return { sold: 0, gold: 0 };
    const inv = this.state.cityInventory.finished;
    if (!inv[item] || inv[item] <= 0) return { sold: 0, gold: 0 };
    inv[item] -= 1;
    const gold = meta.value * Math.pow(1.2, this.state.city?.banks || 0);
