...
class GameEngine {
  ...
  getInitialState() {
    return {
      ...
      autoTransport: {
        // Auto-transport toggles per tier
        basic: false,
        intermediate: false,
        advanced: false,
        premium: false,
  ...
  tick() {
    ...
    if (this.state.resources.gold < 0) {
      console.warn(
        '⚠️ Gold went negative during tick:',
  ...
  updateRDLabsEfficiency() {
    ...
    const rdBonus = Math.pow(1.1, labs) - 1; // Compound 10% per lab
  ...
  updateMiningProduction(deltaTime) {
    ...
    // Get arcade bonuses
    const arcadeBonuses =
      this.arcadeManager && this.arcadeManager.getArcadeBonuses
    ...
    if (
      ...
      this.state.ads.watchedAdBoost.active &&
      now < this.state.ads.watchedAdBoost.endTime
    ) {
    ...
    const applyGatherChance = (baseAmount) => {
      ...
      // Otherwise check for double gather (2x)
        ...
        rebirthEffects.doubleGatherChance > 0 &&
        Math.random() < rebirthEffects.doubleGatherChance
      ) {
        amount *= 2;
      }
  ...
  updateProcessingProduction(deltaTime) {
    ...
    const penaltyProcessing =
      this.state.transport && this.state.transport.penaltyProcessing
        ? this.state.transport.penaltyProcessing
        : 1.0;
    let efficiency =
      this.state.efficiency.processing * penaltyProcessing * this.state.efficiency.global;
  ...
  updateAutomaticTrading(deltaTime) {
    ...
    const penaltyTrading =
      this.state.transport && typeof this.state.transport.penaltyTrading === 'number'
        ? this.state.transport.penaltyTrading
        : 1.0;
    let efficiency =
      this.state.efficiency.trading *
      penaltyTrading *
      (1 - corruption) *
    ...
    if (stoneToSell > 0) {
      ...
      const goldEarned = stoneToSell * 0.1 * Math.pow(1.2, this.state.city.banks);
    ...
    if (coalToSell > 0) {
      ...
      const goldEarned = coalToSell * 0.5 * Math.pow(1.2, this.state.city.banks);
      this.state.resources.gold += goldEarned;
      this.state.stats.totalGoldEarned += goldEarned;
    }
    ...
    if (ironToSell > 0) {
      ...
      const goldEarned = ironToSell * 2 * Math.pow(1.2, this.state.city.banks);
      this.state.resources.gold += goldEarned;
      this.state.stats.totalGoldEarned += goldEarned;
    }
    ...
    if (silverToSell > 0) {
      ...
      const goldEarned = silverToSell * 8 * Math.pow(1.2, this.state.city.banks);
      this.state.resources.gold += goldEarned;
      this.state.stats.totalGoldEarned += goldEarned;
    }
  }
  ...
  updateCityDynamics(deltaTime) {
    ...
    if (this.state.cityInventory && this.state.cityInventory.finished) {
      Object.keys(this.state.cityInventory.finished).forEach((item) => {
        ...
        const itemValue = this.newResourceManager
          ? this.newResourceManager.catalog.finished[item]?.value || 0
          : 0;
    ...
    if (adjustedDecayGeneration > 0) {
      this.state.city.decay = Math.min(
        adjustedMaxDecay,
  ...
  updateTransport() {
    ...
    this.state.transport.capacity =
      baseCapacity * this.state.efficiency.transport * rebirthEffects.transportCapacity;
    ...
    const usage = Math.min(
      100,
      (totalProductionPerSec / Math.max(1, this.state.transport.capacity)) * 100
    );
  ...
  updateUI() {
    ...
    const tub = document.getElementById('transport-usage-bar');
    if (tub) tub.style.width = Math.max(0, Math.min(100, usagePct)) + '%';
    ...
    const bankBonusPct = Math.round((Math.pow(1.2, this.state.city.banks) - 1) * 1000) / 10; // 20% per bank (compound)
    ...
    const salesSpeed =
      salesDept > 0 ? `Level ${salesDept} (sells all every ${interval}s)` : 'Level 0';
    ...
    const miningAcademy = this.state.city.miningAcademy || 0;
    const miningBonus =
      miningAcademy > 0 ? Math.round((Math.pow(1.15, miningAcademy) - 1) * 100) : 0; // Compound 15% per level
    const miningText =
      miningAcademy > 0 ? `Level ${miningAcademy} (+${miningBonus}% gathering)` : 'Level 0';
    ...
    const craftSpeed =
      automationLab > 0 ? `Level ${automationLab} (${automationLab + 1}x speed)` : 'Level 0';
    ...
    const mineEffTotal = Math.round(
      this.state.efficiency.mining * this.state.efficiency.global * 100
    );
    const procEffTotal = Math.round(
      this.state.efficiency.processing * this.state.efficiency.global * 100
    );
    ...
    this.updateElement(
      'crafted-intermediate-count',
      (this.state.crafted?.intermediate || 0).toString()
    );
  ...
  updateInventoryDisplays() {
    ...
    if (factoryDiv) {
      ...
      if (allItems.length === 0) {
        ...
        factoryDiv.innerHTML = allItems
          .map(([item, qty]) => `<div class="inventory-item">${item}: ${Math.floor(qty)}</div>`)
          .join('');
      }
    }
  ...
  updateButtonStates() {
    ...
    this.updateButtonState(
      'hire-coal-miner-btn',
      this.state.resources.gold >= coalCost && this.state.unlock_coal
    );
    this.updateButtonState(
      'hire-iron-miner-btn',
      this.state.resources.gold >= ironCost && this.state.unlock_iron
    );
    this.updateButtonState(
      'hire-silver-miner-btn',
      this.state.resources.gold >= silverCost && this.state.unlock_silver
    );
    ...
    const bd = this.rebirthRewards
      ? this.rebirthRewards.getActiveEffects(this.state.rebirthUpgrades || {}).buildingDiscount
      : 1;
    this.updateButtonState(
      'build-smelter-btn',
      this.state.resources.gold >= Math.ceil(20 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-forge-btn',
      this.state.resources.gold >= Math.ceil(100 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-refinery-btn',
      this.state.resources.gold >= Math.ceil(500 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-mint-btn',
      this.state.resources.gold >= Math.ceil(2000 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-polisher-btn',
      this.state.resources.gold >= Math.ceil(50 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-coker-btn',
      this.state.resources.gold >= Math.ceil(150 * bd) && this.state.unlock_processing
    );
    this.updateButtonState(
      'build-chemplant-btn',
      this.state.resources.gold >= Math.ceil(400 * bd) && this.state.unlock_oil
    );
    this.updateButtonState(
      'build-chipfab-btn',
      this.state.resources.gold >= Math.ceil(1200 * bd) && this.state.unlock_electronics
    );
    this.updateButtonState(
      'build-jeweler-btn',
      this.state.resources.gold >= Math.ceil(900 * bd) && this.state.unlock_jewelry
    );
    this.updateButtonState(
      'build-assembly-btn',
      this.state.resources.gold >= Math.ceil(1600 * bd) && this.state.unlock_electronics
    );
    this.updateButtonState(
      'build-autoplant-btn',
      this.state.resources.gold >= Math.ceil(5000 * bd) && this.state.unlock_automotive
    );
    ...
    this.updateButtonState(
      'hire-stone-trader-btn',
      this.state.resources.gold >= Math.ceil(15 * bd)
    );
    this.updateButtonState('hire-coal-trader-btn', this.state.resources.gold >= Math.ceil(75 * bd));
    this.updateButtonState(
      'hire-metal-trader-btn',
      this.state.resources.gold >= Math.ceil(300 * bd)
    );
    ...
    if (policeBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          policeCount === 0 ? `Hire ${themeName}` : `Hire ${themeName} (Lv ${policeCount})`;
      }
    }
    ...
    if (politicianBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          politicianCount === 0 ? `Hire ${themeName}` : `Hire ${themeName} (Lv ${politicianCount})`;
      }
    }

    // Bank cost scales with 1.20x per building
    const bankCount = this.state.city.banks || 0;
    const bankCost = Math.ceil(500 * Math.pow(1.2, bankCount) * bd);
    ...
    if (bankBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          bankCount === 0 ? `Build ${themeName}` : `Build ${themeName} (Lv ${bankCount})`;
      }
    }
    ...
    if (marketBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          marketCount === 0 ? `Build ${themeName}` : `Build ${themeName} (Lv ${marketCount})`;
      }
    }

    // University cost scales with 1.10x per building
    const universityCount = this.state.city.universities || 0;
    const universityCost = Math.ceil(2500 * Math.pow(1.1, universityCount) * bd);
    ...
    if (universityBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          universityCount === 0
            ? `Build ${themeName}`
            : `Build ${themeName} (Lv ${universityCount})`;
      }
    }
    ...
    if (salesBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          salesDeptLevel === 0
            ? `Build ${themeName}`
            : `Upgrade ${themeName} (Lv ${salesDeptLevel})`;
      }
    }
    ...
    this.updateButtonState(
      'build-mining-academy-btn',
      this.state.resources.gold >= miningAcademyCost
    );
    ...
    if (miningAcademyBtn) {
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          miningAcademyLevel === 0
            ? `Build ${themeName}`
            : `Upgrade ${themeName} (Lv ${miningAcademyLevel})`;
      }
    }
    ...
    this.updateButtonState(
      'build-automation-lab-btn',
      this.state.resources.gold >= automationLabCost
    );
      ...
      if (titleDiv && this.rebirthThemes) {
        ...
        titleDiv.textContent =
          automationLabLevel === 0
            ? `Build ${themeName}`
            : `Upgrade ${themeName} (Lv ${automationLabLevel})`;
      }
    }
    ...
    this.updateButtonState(
      'craft-intermediate-btn',
      (this.state.crafted?.basic || 0) >= 1 &&
        this.state.resources.coal >= 5 &&
        this.state.unlock_coal
    );
    this.updateButtonState(
      'craft-advanced-btn',
      (this.state.crafted?.intermediate || 0) >= 1 &&
        this.state.resources.iron >= 3 &&
        this.state.unlock_iron
    );
    this.updateButtonState(
      'craft-premium-btn',
      (this.state.crafted?.advanced || 0) >= 1 &&
        this.state.resources.silver >= 2 &&
        this.state.unlock_silver
    );

    // Auto-craft toggle (unlocked if ANY auto-craft tier is unlocked)
    const hasAnyAutoCraft =
      this.state.unlock_autocraft_basic ||
    ...
    this.updateButtonState(
      'sell-city-intermediate-btn',
      (cityInv[themeItemNames.intermediate] || 0) >= 1
    );
    ...
    this.updateButtonState(
      'unlock-autocraft-basic-btn',
      this.state.resources.gold >= 100 && !this.state.unlock_autocraft_basic
    );
    this.updateButtonState(
      'unlock-autocraft-intermediate-btn',
      this.state.resources.gold >= 500 &&
        !this.state.unlock_autocraft_intermediate &&
        this.state.unlock_coal
    );
    this.updateButtonState(
      'unlock-autocraft-advanced-btn',
      this.state.resources.gold >= 2500 &&
        !this.state.unlock_autocraft_advanced &&
        this.state.unlock_iron
    );
    this.updateButtonState(
      'unlock-autocraft-premium-btn',
      this.state.resources.gold >= 10000 &&
        !this.state.unlock_autocraft_premium &&
        this.state.unlock_silver
    );
    ...
    const unlockKeys = [
      ...
      'unlock_jewelry',
      'unlock_automotive',
      'unlock_autocraft_basic',
      'unlock_autocraft_intermediate',
      'unlock_autocraft_advanced',
      'unlock_autocraft_premium',
    ];
    ...
    const now = Date.now();
    this.updateButtonState(
      'ad-double-resources-btn',
      this.state.ads.doubleResources.cooldown <= now
    );
    this.updateButtonState(
      'ad-convert-resources-btn',
      this.state.ads.convertResources.cooldown <= now
    );
    this.updateButtonState('ad-gold-bonus-btn', this.state.ads.goldBonus.cooldown <= now);
    this.updateButtonState(
      'ad-efficiency-boost-btn',
      this.state.ads.efficiencyBoost.cooldown <= now
    );
  ...
  getCampaignSnapshot(theme = null) {
    const currentTheme =
      theme || (this.rebirthThemes ? this.rebirthThemes.getCurrentTheme(this.state) : null);
    ...
    if (unlockedCoreCount < coreUnlockKeys.length) {
      ...
      description =
        'The first run is about proving the chain works: unlock every core tier, then make the factory hum.';
      ...
      description =
        'This is where the game becomes infrastructure: research, city systems, and automation start carrying the run.';
      ...
      description =
        'Decay is not failure here. It is the mechanism that turns one strong run into lasting progress.';
      progressLabel = 'Rebirth progress';
      progressValue = decayProgress;
      progressNote =
        'Fill the city with decay, trigger the reset, and cash in the permanent upgrades.';
      nextGoal = decayProgress < 1 ? 'Raise city decay to 100%' : 'Trigger your first rebirth';
      nextReward =
        'The first rebirth opens the long arc of the game, where momentum compounds instead of vanishing.';
    }
      ...
      objectives: [
        {
          ...
          status:
            phase === 'foundation'
              ? 'current'
              : unlockedCoreCount >= coreUnlockKeys.length
                ? 'complete'
                : 'upcoming',
        ...
        },
        {
          ...
          status:
            phase === 'automation'
              ? 'current'
              : completedResearch >= 4 && citySystemsCount >= 3
                ? 'complete'
                : 'upcoming',
        ...
        },
        {
          title: 'Legacy',
          detail:
            'Let the city decay, rebirth the run, and convert the reset into permanent strength.',
          status:
            phase === 'legacy'
              ? 'current'
              : rebirths >= 1 && decayProgress >= 1
                ? 'complete'
                : 'upcoming',
          progress: phase === 'legacy' ? Math.min(1, rebirths / 10) : decayProgress,
          hint:
            rebirths > 0
              ? `${rebirths} rebirths, decay ${Math.round(decayProgress * 100)}%`
              : 'First rebirth unlocks the arc',
        },
      ],
    };
  }
  ...
  updateCampaignUI(snapshot) {
    ...
    this.updateElement(
      'campaign-legacy-chip',
      `Legacy Score: ${this.formatNumber(snapshot.legacyScore)}`
    );
    ...
    if (objectivesContainer) {
      objectivesContainer.innerHTML = snapshot.objectives
        .map((objective) => {
          const stateLabel =
  ...
  mineStone() {
    ...
    if (btn) {
      ...
      const comboText = this._comboCount >= 10 ? '+1 ⚡' : this._comboCount >= 5 ? '+1 🔥' : '+1';
      const color =
        this._comboCount >= 10 ? '#ffd700' : this._comboCount >= 5 ? '#ff6b6b' : '#8d8d8d';
  ...
  watchAd() {
    ...
    if (now < this.state.watchedAdCooldown) {
      ...
      this.showNotification(
        `⏳ Ad available in ${remainingMin} minute${remainingMin > 1 ? 's' : ''}`
      );
      return;
    }
  ...
  craftItem(tier) {
    ...
    const isFree =
      rebirthEffects.freeCraftChance > 0 && Math.random() < rebirthEffects.freeCraftChance;
    ...
    this.state.crafted[recipe.produces] =
      (this.state.crafted[recipe.produces] || 0) + produceAmount;
  ...
  toggleTransportCrafted(tier) {
    ...
    const shortName =
      resultName.split(' ').length > 2 ? resultName.split(' ').slice(0, 2).join(' ') : resultName;
  ...
  tryAutoTransportCrafted() {
    ...
    const sortedTiers = Object.keys(tierInfo).sort(
      (a, b) => tierInfo[b].priority - tierInfo[a].priority
    );
  ...
  sellCraftedFromCity(tier) {
    ...
    const bankBonus = Math.pow(1.2, this.state.city.banks);
  ...
  tryAutoCraft() {
    ...
    if (
      rebirthEffects.instantCraftChance > 0 &&
      Math.random() < rebirthEffects.instantCraftChance
    ) {
  ...
  buildProcessor(processorType) {
    ...
    if (this.state.resources.gold >= cost) {
      ...
      const btnMap = {
        smelter: 'build-smelter-btn',
        forge: 'build-forge-btn',
        refinery: 'build-refinery-btn',
        mint: 'build-mint-btn',
        polishers: 'build-polisher-btn',
        cokers: 'build-coker-btn',
        chemPlants: 'build-chemplant-btn',
        chipFabs: 'build-chipfab-btn',
        jewelers: 'build-jeweler-btn',
        assemblies: 'build-assembly-btn',
  ...
  hireService(serviceType) {
    ...
    if (serviceType === 'police') {
      ...
      const cost = Math.ceil(
        5000 * Math.pow(1.25, currentPolice) * rebirthEffects.buildingDiscount
      ); // 25% increase per hire
      if (this.state.resources.gold >= cost) {
        this.state.resources.gold -= cost;
        this.state.stats.totalGoldSpent += cost;
        ...
        console.log(
          `Hired security guard #${this.state.city.police} for ${cost} gold (upkeep: 0.5 gold/sec or 30 gold/min, generates decay)`
        );
        return true;
      }
      ...
      const cost = Math.ceil(
        250 * Math.pow(1.05, currentPoliticians) * rebirthEffects.buildingDiscount
      );
      if (this.state.resources.gold >= cost) {
        this.state.resources.gold -= cost;
        this.state.stats.totalGoldSpent += cost;
        ...
        if (politicianBtn) {
          ...
          this.spawnFloatingNumber(
            politicianBtn,
            `🏛️ Politician #${this.state.city.politicians}!`,
            '#ffecd2'
          );
        ...
        console.log(
          `Hired politician #${this.state.city.politicians} for ${cost} gold (reduces tax rate by 2% compound)`
        );
        return true;
      }
    }
      ...
      Object.keys(inv).forEach((k) => {
        ...
        if (amount > 0) {
          ...
          if (craftedItems[k]) {
            ...
            const meta =
              this.newResourceManager.catalog.processed[k] ||
  ...
  buildCity(buildingType) {
    ...
    const buildingConfig = {
      bank: { cost: 500, stateKey: 'banks', effectMultiplier: 1.2 }, // 20% bonus per bank
      market: { cost: 1000, stateKey: 'markets', effectMultiplier: 1.15 }, // 15% bonus per market
      university: { cost: 2500, stateKey: 'universities', effectMultiplier: 1.1 }, // 10% bonus per university
    ...
    if (upgradableBuildings.includes(buildingType)) {
      ...
      const cost = Math.ceil(
        config.cost *
          Math.pow(config.effectMultiplier, currentLevel) *
          rebirthEffects.buildingDiscount
      );

      console.log(
        `${buildingType}: Current level ${currentLevel}, Cost: ${cost}, Gold: ${this.state.resources.gold}`
      );
      ...
      if (this.state.resources.gold >= cost) {
        ...
        if (upgBtnId) {
          ...
          if (upgBtn) {
            this.spawnFloatingNumber(
              upgBtn,
              `⭐ Lvl ${this.state.city[config.stateKey]}!`,
              '#ffd700'
            );
    ...
    if (this.state.resources.gold >= cost) {
      ...
      if (cityBtnId) {
        ...
        if (cityBtn) {
          ...
          this.spawnFloatingNumber(
            cityBtn,
            `${buildEmojis[buildingType]} #${this.state.city[config.stateKey]}!`,
            '#fcb69f'
          );
      ...
      console.log(
        `Built ${buildingType} level ${this.state.city[config.stateKey]} for ${cost} gold (base: ${config.cost})`
      );
      return true;
    }
    return false;
  ...
  updateRebirthUpgradesUI() {
    ...
    const powerData = this.rebirthRewards.calculateRebirthPower(
      rebirthCount,
      this.state.rebirthUpgrades
    );
    ...
    for (let tier = 1; tier <= 9; tier++) {
      ...
      tierUpgrades.forEach(({ key, ...upgrade }) => {
        ...
        const effectText = isMaxed
          ? this.rebirthRewards
              .getUpgradeDescription(key, currentLevel)
              .replace(/Next:.*/, '')
              .replace(/\|.*/, '')
              .trim()
  ...
  sellResource(resourceType) {
    ...
    if (this.state.resources[resourceType] >= 1) {
      ...
      const goldEarned =
        sellRates[resourceType] *
        Math.pow(1.2, this.state.city.banks) *
        eventSellMult *
        goldCapMult;
  ...
  rebirth() {
    ...
    if (this.state.city.decay < effectiveMaxDecay - 0.5) {
      console.log(
        `Cannot rebirth: decay is ${this.state.city.decay.toFixed(1)}/${effectiveMaxDecay.toFixed(1)}`
      );
      this.showNotification(
        `City must reach 100% decay before rebirth! (${Math.round(this.state.city.decay)}/${Math.round(effectiveMaxDecay)})`
      );
      return false;
    }
    ...
    const stagedGoodsTotal = Object.values(cityFinished).reduce(
    ...
    console.log(
      `🔄 REBIRTH: Starting rebirth #${newRebirthCount}, preserving upgrades:`,
      savedRebirthUpgrades
    );
    ...
    if (savedCityInventory) {
      ...
      console.log(
        `📦 City inventory preserved across rebirth (${Math.floor(stagedGoodsTotal)} staged goods)`
      );
    }
    ...
    console.log(
      `🔄 Efficiency reset to base with rebirth bonuses: mining=${this.state.efficiency.mining}, processing=${this.state.efficiency.processing}`
    );
    ...
    console.log(
      `✅ City rebirth completed! Total rebirths: ${newRebirthCount} - Research, Unlocks, and Arcade state preserved`
    );
...
╚════════════════════════════════════════╝
  ...
  static EVENTS = [
    {
      ...
      emoji: '🐫',
      name: 'Trade Caravan',
      desc: 'A travelling merchant doubles your sell prices!',
      duration: 30,
      effect: 'doubleSellPrice',
    },
    {
      id: 'gold_rush',
      emoji: '💰',
      name: 'Gold Rush',
      desc: 'Workers find gold nuggets - +50% mining speed!',
      duration: 25,
      effect: 'miningBoost',
    },
    {
      ...
      emoji: '⚠️',
      name: 'Mine Collapse',
      desc: 'A tunnel collapsed! Mining slowed by 50% for a while.',
      duration: 20,
      effect: 'miningPenalty',
    },
    {
      id: 'market_boom',
      emoji: '📈',
      name: 'Market Boom',
      desc: 'Resource prices surge - sell now for 3× value!',
      duration: 20,
      effect: 'tripleSellPrice',
    },
    {
      id: 'lucky_find',
      emoji: '🍀',
      name: 'Lucky Find',
      desc: 'Workers discovered a rich vein! Free bonus resources.',
      duration: 0,
      effect: 'freeResources',
    },
    {
      id: 'tax_holiday',
      emoji: '🎉',
      name: 'Tax Holiday',
      desc: 'The government declared a tax holiday!',
      duration: 45,
      effect: 'noTax',
    },
    {
      ...
      desc: 'Everything runs faster for a short burst!',
      duration: 30,
      effect: 'globalBoost',
    },
    {
      ...
      emoji: '✊',
      name: 'Worker Strike',
      desc: 'Workers demand better pay - production halved!',
      duration: 15,
      effect: 'halfProduction',
    },
    {
      id: 'merchant_visit',
      emoji: '🏪',
      name: 'Wandering Merchant',
      desc: 'A merchant offers to buy coal at 5× price!',
      duration: 20,
      effect: 'coalBonus',
    },
    {
      ...
      emoji: '🌪️',
      name: 'Decay Storm',
      desc: 'A surge of entropy - decay jumps forward!',
      duration: 0,
      effect: 'decayBoost',
    },
  ];
  ...
  showEventBanner(event) {
    ...
    if (!banner) {
      ...
      if (!document.getElementById('event-anim-style')) {
        ...
        s.textContent =
          '@keyframes eventSlide{from{opacity:0;transform:translateX(-50%) translateY(-30px);}to{opacity:1;transform:translateX(-50%) translateY(0);}}';
  ...
  static MILESTONES = [
    {
      id: 'stone_1k',
      check: (s) => s.resources.stone >= 1000,
      msg: '🪨 1,000 Stone - Quarry Master!',
    },
    {
      id: 'stone_1m',
      check: (s) => s.resources.stone >= 1e6,
      msg: '⛰️ 1M Stone - Mountain Mover!',
    },
    ...
    {
      id: 'gold_1b',
      check: (s) => s.resources.gold >= 1e9,
      msg: '👑 1B Gold - Billionaire Tycoon!',
    },
    {
      id: 'workers_10',
      check: (s) =>
        s.workers.stoneMiners +
          s.workers.coalMiners +
          s.workers.ironMiners +
          s.workers.silverMiners >=
        10,
      msg: '👷 10 Workers - Small Team!',
    },
    {
      id: 'workers_50',
      check: (s) =>
    ...
    },
    {
      ...
      check: (s) =>
        s.workers.stoneMiners +
          ...
          s.workers.silverMiners >=
        200,
      msg: '🌐 200 Workers - Empire!',
    },
    {
      id: 'rebirth_1',
      check: (s) => s.city.rebirths >= 1,
      msg: '🔄 First Rebirth - New Beginning!',
    },
    ...
    {
      id: 'silver_unlock',
      check: (s) => s.unlock_silver,
      msg: '🥈 Silver Unlocked - Precious Metals!',
    },
    {
      id: 'event_5',
      check: (s) => (s.events?.eventsTriggered || 0) >= 5,
      msg: '🎲 5 Events Survived!',
    },
    {
      id: 'event_20',
      check: (s) => (s.events?.eventsTriggered || 0) >= 20,
      msg: '🎰 20 Events - Veteran!',
    },
    { id: 'iron_100k', check: (s) => s.resources.iron >= 1e5, msg: '⚙️ 100K Iron - Forge Lord!' },
    {
      id: 'silver_10k',
      check: (s) => s.resources.silver >= 1e4,
      msg: '🪙 10K Silver - Silver Baron!',
    },
  ];
  ...
  applyPrestigeBonus(target, cost) {
    ...
    const bonusCosts = {
      ...
      processing: Math.max(
        1,
        Math.floor(200 * Math.pow(2, this.state.prestige.bonuses.processing || 0))
      ),
      trading: Math.max(1, Math.floor(300 * Math.pow(2, this.state.prestige.bonuses.trading || 0))),
      transport: Math.max(
        1,
        Math.floor(500 * Math.pow(2, this.state.prestige.bonuses.transport || 0))
      ),
    ...
    if (this.state.prestige.points >= bonusCosts[target]) {
      ...
      console.log(
        `Applied ${target} prestige bonus! New level: ${this.state.prestige.bonuses[target]}`
      );
    }
  }
  ...
  showNotification(message, tone = 'default') {
    ...
    notification.className =
      tone === 'achievement' ? 'game-notification achievement' : 'game-notification';
