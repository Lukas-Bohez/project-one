...
class MarketSystem {
  ...
  hireMarketer(tier) {
    ...
    console.log(
      `Hired ${marketerInfo.name}. New sell multiplier: ${this.calculateSellMultiplier().toFixed(1)}x`
    );
    return true;
  }
  ...
  getMarketStats() {
    ...
    const totalMarketers =
      this.state.basicMarketers + this.state.advancedMarketers + this.state.expertMarketers;
  ...
  getMarketerRecommendations() {
    ...
    Object.keys(this.marketerTiers).forEach((tier) => {
      ...
      if (tier === 'basic' && this.state.basicMarketers < 3) {
        ...
        tier === 'advanced' &&
        this.state.basicMarketers >= 2 &&
        this.state.advancedMarketers < 2
      ) {
        priority = 'medium';
  ...
  getOptimalMarketerDistribution(budget) {
    ...
    const efficiencies = Object.keys(this.marketerTiers)
      .map((tier) => ({
        tier,
