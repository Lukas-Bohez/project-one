...
class SaveManager {
  ...
  async saveGame(gameState = null, forceReset = false) {
    if (!this.isAuthenticated) {
      this.gameEngine.showNotification(
        '⚠️ You must be logged in to save your game! Click "Login" to create an account or sign in.'
      );
  ...
  async loadGame() {
    ...
    try {
      ...
      console.log(
        '🔍 LOAD: save_data keys:',
        data?.save_data ? Object.keys(data.save_data) : 'N/A'
      );
      ...
      if (response.status === 404 || (response.ok && data && data.has_save === false)) {
        ...
        console.log(
          '✅ LOAD: Rebirth count in saved data:',
          data.save_data?.custom_data?.city?.rebirths
        );
  ...
  prepareSaveData() {
    ...
    return {
      ...
      custom_data: {
        ...
        autoTransport: state.autoTransport || {
          basic: false,
          intermediate: false,
          advanced: false,
          premium: false,
  ...
  applySaveData(saveData) {
    ...
    if (customData.city) {
      ...
      if (currentRebirths > state.city.rebirths) {
        console.warn(
          `⚠️ REBIRTH PROTECTION: Saved rebirth (${state.city.rebirths}) < current (${currentRebirths}), keeping current`
        );
    ...
    if (customData.arcade) {
      ...
      state.arcade.unlockedGames = customData.arcade.unlockedGames || [
        'doom',
        'digger',
        'commander',
        'prince',
    ...
    if (typeof state.transport.penaltyProcessing !== 'number')
      state.transport.penaltyProcessing = 1.0;
  ...
  async getLeaderboard(limit = 100) {
    try {
      ...
      const response = await fetch(
        `${this.baseUrl}${this.apiEndpoint}/leaderboard?limit=${limit}`,
        {
          method: 'GET',
          headers: {
            Authorization: `Bearer ${this.authToken}`,
