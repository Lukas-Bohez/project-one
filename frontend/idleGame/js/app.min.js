class IndustrialEmpireApp {
  ...
  initializeGameEngine() {
    ((this.gameEngine = new GameEngine()), console.log('Game engine initialized'));
  }
  setupEventListeners() {
    (this.setupActionListeners(), console.log('Event listeners set up'));
  }
  setupTabs() {
    (document.querySelectorAll('.tab-btn').forEach((e) => {
      e.addEventListener('click', (e) => {
        const t = e.currentTarget.dataset.tab || e.target.closest('.tab-btn')?.dataset.tab;
        (this.switchTab(t), 'arcade' === t && this.updateArcadeUI());
      });
    }),
  ...
  updateArcadeUI() {
    ...
    const e = this.gameEngine.arcadeManager,
      t = document.getElementById('arcade-games-container');
    if (!t) return;
    t.innerHTML = '';
    (e.getAvailableGames().forEach((n) => {
      const i = document.createElement('button');
      ((i.className = n.unlocked
        ? 'action-btn success-btn'
        : n.canUnlock
          ? 'action-btn'
          : 'action-btn disabled'),
        (i.disabled = !n.unlocked && !n.canUnlock));
      ...
      if (
        ...
        ((t.className = 'btn-cost'), (t.textContent = `Played: ${n.playTime}`), i.appendChild(t));
        const a = document.createElement('div');
        a.className = 'btn-description';
        const s = (e.state.arcade.playTime[n.id] || 0) / 3600,
          o = Math.min(10, s),
          r = Math.round(o * n.bonusAmount * 100);
        ((a.textContent = `${n.description} | Bonus: +${r}%`),
          i.appendChild(a),
          (i.onclick = () => openDosGame(n.id)));
      } else {
        const t = document.createElement('div');
        ((t.className = 'btn-cost'),
          (t.textContent = `Unlock: ${n.unlockCost} gold`),
          i.appendChild(t));
        const a = document.createElement('div');
        ((a.className = 'btn-description'),
          ...
          n.canUnlock &&
            (i.onclick = () => {
              e.unlockGame(n.id) && (this.updateArcadeUI(), this.updateArcadeStats());
            }));
      }
      t.appendChild(i);
    }),
  ...
  updateArcadeStats() {
    if (!this.gameEngine || !this.gameEngine.arcadeManager) return;
    const e = this.gameEngine.arcadeManager,
      t = e.getArcadeBonuses(),
      n = e.state.arcade.totalPlayTime,
      i = Math.floor(n / 3600),
      a = Math.floor((n % 3600) / 60),
      s = document.getElementById('arcade-total-time');
    s && (s.textContent = `${i}h ${a}m`);
    const o = document.getElementById('arcade-resource-bonus');
    o && (o.textContent = `+${Math.round(100 * (t.resourceBonus - 1))}%`);
    const r = document.getElementById('arcade-efficiency-bonus');
    r && (r.textContent = `+${Math.round(100 * (t.efficiencyBonus - 1))}%`);
    const d = document.getElementById('arcade-gold-bonus');
    d && (d.textContent = `+${Math.round(100 * (t.goldBonus - 1))}%`);
    const c = document.getElementById('arcade-crafting-bonus');
    c && (c.textContent = `+${Math.round(100 * (t.craftingBonus - 1))}%`);
    const m = e.getMaterialsGathered(), g = document.getElementById('arcade-materials');
    if (g) { const u = Object.entries(m);
      if (u.length === 0) g.innerHTML = '<div class="resource-row"><span class="resource-type">Open a game to start gathering!</span><span class="resource-count" style="color:#94a3b8;">—</span></div>';
      else { const l = { stone: '🪨 Stone', coal: '🪨 Coal', iron: '⚙️ Iron', silver: '🥈 Silver', gold: '🪙 Gold', workers: '👷 Workers hired' };
        g.innerHTML = u.sort((a, b) => b[1] - a[1]).map(([t, a]) => `<div class="resource-row"><span class="resource-type">${l[t] || t}:</span><span class="resource-count">${a}</span></div>`).join(''); } }
  }
  switchTab(e) {
    ...
    (t && t.classList.add('active'), n && n.classList.add('active'), (this.currentTab = e));
    const i = this.gameEngine?.themeManager;
    'arcade' === e &&
      i &&
      i.currentTheme &&
      'sterile' === i.currentTheme.atmosphere &&
      setTimeout(() => i.fixArcadeTextForSterile(), 50);
  }
  setupActionListeners() {
    ...
    (document.querySelectorAll('[data-unlock]').forEach((e) => {
      e.addEventListener('click', () => {
        const t = e.getAttribute('data-unlock');
        this.gameEngine.unlockFeature(t);
      });
    }),
      ...
      this.bindButton('research-logistics-btn', () =>
        this.gameEngine.purchaseResearch('logistics')
      ),
      ...
      this.bindButton('ad-efficiency-boost-btn', () =>
        this.gameEngine.watchAdForEfficiencyBoost()
      ));
  }
  bindButton(e, t) {
...
(document.addEventListener('DOMContentLoaded', async () => {
  (console.log('DOM loaded, initializing Industrial Empire...'),
    ...
    setTimeout(() => {
      window.DeveloperMenu && window.app.gameEngine
