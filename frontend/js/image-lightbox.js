(function () {
  ...
  function applyTransform() {
    imgEl.style.transform =
      'scale(' + curZoom + ') translate(' + panX / curZoom + 'px,' + panY / curZoom + 'px)';
  ...
  function onTouchTap(e) {
    var now = Date.now();
    if (now - lastTap < 300) {
      onDblClick({ clientX: e.changedTouches[0].clientX, clientY: e.changedTouches[0].clientY });
    }
    lastTap = now;
  }
  ...
  function buildOverlay() {
    ...
    zoomLabel.style.cssText =
      'color:#fff;font-size:13px;font-weight:700;min-width:38px;text-align:center';
    zoomLabel.textContent = '1.0\u00d7';

    bar.appendChild(
      makeBtn('ilb-zo', '− Out', 'Zoom out', function () {
        zoomBy(-0.5, window.innerWidth / 2, window.innerHeight / 2);
      })
    );
    bar.appendChild(zoomLabel);
    bar.appendChild(
      makeBtn('ilb-zi', '+ In', 'Zoom in', function () {
        zoomBy(0.5, window.innerWidth / 2, window.innerHeight / 2);
      })
    );
    bar.appendChild(
      makeBtn('ilb-rs', '↺ Reset', 'Reset zoom', function () {
        resetZoom();
      })
    );
    bar.appendChild(
      makeBtn('ilb-fs', '⛶', 'Fullscreen', function () {
        toggleFullscreen();
      })
    );
    bar.appendChild(
      makeBtn('ilb-cl', '✕ Close', 'Close', function () {
        close();
      })
    );
  ...
  function init() {
    ...
    var imgs = document.querySelectorAll(
      '[data-zoomable], .proof-screenshot img, .screenshot img, .gallery img, .project-screenshot, .c-proof-gallery__item img'
    );
    ...
    var triggers = document.querySelectorAll(
      '[data-lightbox-src], [data-screenshot-src], [data-proof-open]'
    );
