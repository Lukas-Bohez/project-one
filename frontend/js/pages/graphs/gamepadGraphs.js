class GamepadNavigator {
  ...
  updateNavigableElements() {
    ...
    const gamepadElements = document.querySelectorAll(
      '.gamepad, .gamepad-chart-container, .toggle-container'
    );
    ...
    this.navigableElements.forEach((el) => {
      ...
      if (
        el.classList.contains('toggle-container') &&
        !el.querySelector('.gamepad-scroll-indicator')
      ) {
      ...
      case 'up':
        if (rect2.bottom < rect1.top) {
          distance =
            rect1.top -
            rect2.bottom +
            Math.abs(rect1.left + rect1.width / 2 - (rect2.left + rect2.width / 2));
        } else {
          distance = Infinity;
        }
        break;
      ...
      case 'left':
        if (rect2.right < rect1.left) {
          distance =
            rect1.left -
            rect2.right +
            Math.abs(rect1.top + rect1.height / 2 - (rect2.top + rect2.height / 2));
        } else {
          distance = Infinity;
        }
        break;
  ...
  updateSelection() {
    ...
    if (
      this.gamepadIndex !== null &&
      this.navigableElements.length > 0 &&
      this.navigableElements[this.currentSelectedIndex]
    ) {
      const selectedElement = this.navigableElements[this.currentSelectedIndex];
