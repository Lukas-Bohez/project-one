...
async function loadShifts() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE}/shifts?business_id=${currentBusiness.id}&start_date=${startDate}&end_date=${endDateStr}`,
      {
        headers: headers,
      }
    );
...
function renderCalendar() {
  if (employees.length === 0) {
    calendarBody.innerHTML =
      '<div class="loading-state"><p>No employees found. Add employees to start scheduling.</p></div>';
    return;
  }
  ...
  employees.forEach((employee) => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const employeeShifts = shifts.filter(
        (s) => s.employee_id === employee.id && s.shift_date === dateStr
      );
...
async function handleQuickFillSubmit(e) {
  ...
  const checkedDays = Array.from(document.querySelectorAll('input[name="days"]:checked')).map(
