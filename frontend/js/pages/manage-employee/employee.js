...
async function loadShifts() {
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE}/employees/${currentEmployee.id}/shifts?start_date=${startDate}&end_date=${endDate}`,
      {
        headers: headers,
      }
    );
...
function updateProfileInfo() {
  ...
  document.getElementById('profileRate').textContent =
    `$${currentEmployee.hourly_rate.toFixed(2)}/hr`;
  document.getElementById('profileHireDate').textContent = formatDateShort(
...
function calculateTimeOff() {
  ...
  if (startDate && endDate && endDate >= startDate) {
    ...
    document.getElementById('remainingPTO').textContent = Math.max(
      0,
      currentEmployee.pto_balance - hours
    );
  }
}
