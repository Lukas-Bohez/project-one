...
class StoryWeaver {
  ...
  loadExampleStory(path = '/exampleStories/enchantedForest.json', suppressConfirm = false) {
    ...
    fetch(path)
      ...
      .catch((err) => {
        console.warn('Failed to load example story:', err);
        document.body.classList.remove('loading');
      });
  }
  ...
  renderEditor() {
    ...
    // Update page title
    try {
      document.title = this.story.meta.title + ' - Story Weaver';
    } catch (e) {}
  ...
  renderScenesList() {
    ...
    const scenes = Object.values(this.story.scenes).filter(
      (scene) =>
  ...
  renderChoices(scene) {
    ...
    scene.choices.forEach((choice, index) => {
      ...
      const conditionsBadge = hasConditions
        ? ` <span style="background: var(--primary); color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 600;">${choice.conditions.length}</span>`
        : '';
      const effectsBadge = hasEffects
        ? ` <span style="background: var(--success); color: white; padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: 600;">${choice.effects.length}</span>`
        : '';
  ...
  startNewGame() {
    ...
    if (!this.story.meta.startScene || !this.story.scenes[this.story.meta.startScene]) {
      this.showModal(
        'Cannot Play',
        'No valid starting scene is set. Please choose a starting scene in Settings.'
      );
  ...
  loadPlayScene(sceneId) {
    ...
    if (!scene) {
      this.showModal(
        'Scene Not Found',
        'The target scene could not be found. The story may be incomplete.'
      );
      return;
    }
  ...
  deleteScene(sceneId) {
    ...
    if (this.currentEditingScene === sceneId) {
      this.currentEditingScene = null;
      const sceneEditor =
        document.getElementById('sceneEditor') || document.getElementById('sceneEditorContainer');
  ...
  duplicateScene(sceneId) {
    ...
    if (Array.isArray(duplicatedScene.choices)) {
      duplicatedScene.choices = duplicatedScene.choices.map((choice) => {
        ...
        newChoice.id =
          'choice_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 6);
        return newChoice;
      });
    }
  ...
  exportStory() {
    ...
    link.href = url;
    const safeTitle =
      this.story.meta && this.story.meta.title
        ? this.story.meta.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()
        : 'story';
  ...
  renderDashboard() {
    ...
    const choiceCount = Object.values(this.story.scenes).reduce(
      (total, scene) => total + (scene.choices ? scene.choices.length : 0),
      0
    );
  ...
  validateStory() {
      ...
      if (scene.choices) {
        scene.choices.forEach((choice) => {
          if (choice.target && !this.story.scenes[choice.target]) {
            errors.push(
              `Choice "${choice.text}" in scene "${scene.title}" points to non-existent scene`
            );
          }
        });
      }
    ...
    Object.keys(this.story.scenes).forEach((sceneId) => {
      if (!reachableScenes.has(sceneId) && sceneId !== this.story.meta.startScene) {
        warnings.push(
          `Scene "${this.story.scenes[sceneId].title}" is not reachable from the starting scene`
        );
      }
    });
    ...
    if (errors.length === 0 && warnings.length === 0) {
      report +=
        '<p style="color: var(--primary-solid); font-weight: bold;">🎉 Your story is valid with no issues found!</p>';
    } else {
      if (errors.length > 0) {
      ...
      if (warnings.length > 0) {
        ...
        warnings.forEach(
          (warning) => (report += `<li style="margin-bottom: 4px;">${warning}</li>`)
        );
        report += '</ul>';
      }
    }
    ...
    this.showModal('Story Validation', report, [{ text: 'OK', action: 'close' }]);
  }
  ...
  editChoiceConditions(choiceId) {
    ...
    const conditionsHTML =
      choice.conditions
            ...
            <div class="choice-item" style="margin-bottom: 12px; padding: 12px; background: var(--surface-hover); border-radius: var(--radius-md);">
                <div style="display: flex; gap: 8px; margin-bottom: 8px;">
                    <select class="form-select" style="flex: 1;" data-index="${index}" data-field="variable" onchange="app.updateConditionField('${choiceId}', ${index}, 'variable', this.value)">
                        ${Object.keys(this.story.variables)
                          .map(
                            (varName) =>
        ...
        .join('') ||
      '<p style="color: var(--text-secondary); text-align: center;">No conditions yet</p>';
            ...
            <div style="margin-bottom: 16px;">
                ...
                <div style="display: flex; gap: 8px;">
                    <select id="newConditionVar" class="form-select" style="flex: 1;">
                        ${Object.keys(this.story.variables)
                          .map((varName) => `<option value="${varName}">${varName}</option>`)
                          .join('')}
                    </select>
    ...
    this.showModal('Edit Conditions', content, [{ text: 'Done', action: 'close' }]);
  }
  ...
  editChoiceEffects(choiceId) {
    ...
    const effectsHTML =
      choice.effects
        ...
        .join('') ||
      '<p style="color: var(--text-secondary); text-align: center;">No effects yet</p>';
    ...
    this.showModal('Edit Effects', content, [{ text: 'Done', action: 'close' }]);
  }
