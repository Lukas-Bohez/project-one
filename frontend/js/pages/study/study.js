...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const cache = {
    get: function (key) {
      try {
        ...
        if (typeof Storage !== 'undefined' && localStorage) {
          ...
          if (item) {
            ...
            console.log(
              `[Cache] Retrieved from localStorage for key: ${key}, timestamp: ${new Date(parsed.timestamp).toISOString()}`
            );
            return parsed;
          }
        }
        ...
        try {
          ...
          console.warn(
            `[Cache] Could not clear corrupted localStorage item: ${clearError.message}`
          );
        }
      }
      ...
      if (memoryResult) {
        console.log(
          `[Cache] Retrieved from memory for key: ${key}, timestamp: ${new Date(memoryResult.timestamp).toISOString()}`
        );
      }
      return memoryResult;
    },
    ...
    set: function (key, data) {
      ...
      try {
        ...
        if (typeof Storage !== 'undefined' && localStorage) {
          ...
          console.log(
            `[Cache] Data saved to localStorage for key: ${key}, timestamp: ${new Date(cacheData.timestamp).toISOString()}`
          );
          return;
        }
      } catch (e) {
      ...
      console.log(
        `[Cache] Data saved to memory cache for key: ${key}, timestamp: ${new Date(cacheData.timestamp).toISOString()}`
      );
    },
  ...
  const fetchThemes = async () => {
    ...
    try {
      ...
      if (cachedData && !bypassCache) {
        if (cache.isValid(cachedData, CACHE_DURATION)) {
          console.log(
            `[Cache Hit] Using cached themes, age: ${Date.now() - cachedData.timestamp}ms`
          );
          return cachedData.data;
        } else {
  ...
  const updateQuestionSelectorBasedOnFilters = () => {
    ...
    try {
      ...
      const selectedThemeIds = dom.themeFilterContainer
        ? Array.from(dom.themeFilterContainer.querySelectorAll('input:checked')).map(
            (cb) => cb.dataset.id
          )
        : [];
      const selectedDifficultyIds = dom.difficultyFilterContainer
        ? Array.from(dom.difficultyFilterContainer.querySelectorAll('input:checked')).map(
            (cb) => cb.dataset.difficultyId
          )
        : [];
      ...
      const filteredQuestions = allQuestions.filter((q) => {
        const themeMatch =
          selectedThemeIds.length === 0 || selectedThemeIds.includes(String(q.themeId));
        const difficultyMatch =
          selectedDifficultyIds.length === 0 ||
          selectedDifficultyIds.includes(String(q.difficultyLevelId));
        return themeMatch && difficultyMatch;
      });

      console.log(
        `Filtered ${filteredQuestions.length} questions from ${allQuestions.length} total`
      );
  ...
  const displayQuestion = (index) => {
    ...
    if (dom.difficultyTag) {
      ...
      dom.difficultyTag.textContent =
        difficultyMap[question.difficultyLevelId] || question.difficulty || 'Normal';
    }
    if (dom.questionText)
      dom.questionText.textContent = question.question_text || question.questionText;
    ...
    if (dom.nextQuestionBtn) {
      dom.nextQuestionBtn.textContent =
        index === practiceQuestions.length - 1 ? 'Finish Practice' : 'Next';
    }
  };
  ...
  const handleCheckAnswer = () => {
    ...
    answerButtons.forEach((btn) => {
      ...
      if (
        ...
        answerData &&
        !(answerData.is_correct || answerData.isCorrect)
      ) {
    ...
    if (dom.explanation) {
      ...
      dom.explanation.appendChild(
        document.createTextNode(currentQuestion.explanation || 'No explanation available.')
      );
  ...
  const handleNextPrev = (direction) => {
    ...
    if (direction === 1 && newIndex >= practiceQuestions.length) {
      ...
      if (dom.questionDisplay) {
        dom.questionDisplay.innerHTML =
          '<h2>Practice Complete! ✅</h2><p>Select new questions or filters to start another session.</p>';
      }
    } else if (newIndex >= 0) {
  ...
  function initializeQuestionSelectorLogic() {
    ...
    if (dom.practiceSelectedBtn) {
      ...
      dom.practiceSelectedBtn.parentNode.replaceChild(
        newPracticeSelectedBtn,
        dom.practiceSelectedBtn
      );
  ...
  const setupEventListeners = () => {
    ...
    if (deselectAllFiltersBtn) {
      deselectAllFiltersBtn.addEventListener('click', () => {
        ...
        if (dom.themeFilterContainer) {
          dom.themeFilterContainer
            .querySelectorAll('input[type="checkbox"]')
            .forEach((cb) => (cb.checked = false));
        }
        // Deselect all difficulty filters
        if (dom.difficultyFilterContainer) {
          dom.difficultyFilterContainer
            .querySelectorAll('input[type="checkbox"]')
            .forEach((cb) => (cb.checked = false));
        }
        updateQuestionSelectorBasedOnFilters();
  ...
  const main = async () => {
    ...
    try {
      ...
      if (allQuestions.length === 0) {
        ...
        if (dom.questionDisplay) {
          dom.questionDisplay.innerHTML =
            '<h2>No Questions Available</h2><p>No active questions found. Please check your API connection.</p>';
          dom.questionDisplay.style.display = 'block';
        }
        return;
      }
      ...
      console.log(
        'Themes loaded:',
        themesWithCounts.map((t) => `${t.name} (${t.questionCount.count})`)
      );
  ...
  console.log(
    '%c[Cache Info] Cache system active with smart caching:',
