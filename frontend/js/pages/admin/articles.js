...
const loadArticles = async (page = 1) => {
  ...
  try {
    ...
    if (articlesState.filters.storyId) {
      ...
      const totals = (articlesState.articles || []).reduce(
        (acc, a) => {
          acc.total += 1;
    ...
    articleList.innerHTML =
      '<div class="c-error-state">Error loading articles. Please try again.</div>';
  }
};
...
const renderArticlesGrouped = async () => {
  ...
  try {
    ...
    const perStoryArticles = await Promise.all(
      stories.map(async (s) => {
        try {
    ...
    const totals = perStoryArticles.reduce(
      (acc, group) => {
                    ...
                    ${contentData.cards
                      .map((card) => {
                        ...
                        const imgHtml = card.image
                          ? `<div class="content-card-image"><img src="${escapeHTML(card.image)}" alt="${escapeHTML(card.title || 'Card image')}"></div>`
                          : '';
                        return `
  ...
  if (articlesState.filters.storyId) {
    ...
    const storyDesc =
      story && story.description
        ? `<p class="c-story-desc">${escapeHTML(story.description)}</p>`
        : '';
    ...
    const headerHtml = `
        ...
        `;
    const itemsHtml = articlesState.articles
      .map((article) => createArticleElement(article))
      .join('');
    articleList.innerHTML = headerHtml + itemsHtml;
  } else {
    // Default flat list (keeps pagination behavior intact)
    articleList.innerHTML = articlesState.articles
      .map((article) => createArticleElement(article))
      .join('');
  }
...
const createArticleElement = (article) => {
  ...
  const intro =
    contentData.intro || article.story || article.excerpt || 'No introduction available';
...
const setupArticleFormListeners = () => {
  ...
  document.querySelectorAll('.c-status-toggle-option').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      document
        .querySelectorAll('.c-status-toggle-option')
...
try {
  ...
  console.warn('Articles listeners failed to init immediately:', e);
}
if (document.readyState === 'loading') {
  window.addEventListener('DOMContentLoaded', () => {
    try {
      ...
      console.warn('Articles listeners failed to init on DOMContentLoaded:', e);
    }
  });
}
