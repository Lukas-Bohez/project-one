...
(function () {
  ...
  async function loadCommunityThemes(filter) {
    ...
    if (!container) return;
    container.innerHTML =
      '<div class="c-loading"><i class="fas fa-spinner fa-spin"></i> Loading community themes...</div>';
  ...
  function sortThemes(themes, sort) {
    ...
    switch (sort) {
      ...
      case 'questions':
        return copy.sort((a, b) => (b.question_count || 0) - (a.question_count || 0));
      case 'name':
  ...
  window.showRejectModal = function (themeIdOrIds) {
    ...
    overlay.querySelector('.js-reject-confirm').onclick = async () => {
      ...
      overlay.querySelector('.js-reject-confirm').innerHTML =
  ...
  window.previewCommunityTheme = async function (themeId) {
    try {
      ...
      let questions = [];
      try {
        questions = await communityApi(`/themes/${themeId}/questions`);
      } catch {}
  ...
  window.editCommunityTheme = async function (themeId) {
    try {
      ...
      form.querySelector('.js-save-community-edits').onclick = () =>
        saveCommunityEdits(themeId, false);
