  ...
  try {
    ...
    if (usersCountElements && usersCountElements.length > 0) {
      ...
      document.dispatchEvent(
        new CustomEvent('usersCountUpdated', {
          detail: {
            count,
            timestamp: Date.now(),
            success: true,
    ...
    document.dispatchEvent(
      new CustomEvent('usersCountError', {
        detail: {
          error: error.message,
          timestamp: Date.now(),
        },
    ...
    if (usersRetryCount < MAX_RETRIES) {
      ...
      console.log(
        `Retrying users count in ${retryDelay}ms (attempt ${usersRetryCount}/${MAX_RETRIES})`
      );
...
async function updateActiveQuestionsCount() {
  ...
  try {
    ...
    if (questionsCountElements && questionsCountElements.length > 0) {
      ...
      // Dispatch custom event
      document.dispatchEvent(
        new CustomEvent('questionCountUpdated', {
          detail: {
            count,
            timestamp: Date.now(),
            success: true,
    ...
    document.dispatchEvent(
      new CustomEvent('questionCountError', {
        detail: {
          error: error.message,
          timestamp: Date.now(),
        },
    ...
    if (questionsRetryCount < MAX_RETRIES) {
      ...
      console.log(
        `Retrying questions count in ${retryDelay}ms (attempt ${questionsRetryCount}/${MAX_RETRIES})`
      );
...
async function updateAverageScore() {
  ...
  try {
    ...
    if (averageScoreElements && averageScoreElements.length > 0) {
      ...
      // Dispatch custom event
      document.dispatchEvent(
        new CustomEvent('averageScoreUpdated', {
          detail: {
            percentage: score,
            timestamp: Date.now(),
            success: true,
    ...
    document.dispatchEvent(
      new CustomEvent('averageScoreError', {
        detail: {
          error: error.message,
          timestamp: Date.now(),
        },
    ...
    if (averageScoreRetryCount < MAX_RETRIES) {
      ...
      console.log(
        `Retrying average score in ${retryDelay}ms (attempt ${averageScoreRetryCount}/${MAX_RETRIES})`
      );
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (elements.hasUsers || elements.hasQuestions || elements.hasAverageScore) {
    ...
    console.warn(
      'No elements with classes "js-users-count", "js-question-count", or "js-average-score" found'
    );
  }
});
