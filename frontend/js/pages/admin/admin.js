  ...
  const answerPromises = questions.map(async (question) => {
    try {
      ...
      if (!answersResponse.ok) {
        console.warn(
          `HTTP error fetching answers for question ID ${question.id}! Status: ${answersResponse.status}`
        );
        return [];
      }
  ...
  try {
    ...
    if (cachedDataString) {
      ...
      if (now - timestamp >= CACHE_DURATION) {
        console.log(
          `[Cache Background Update] Cache is stale, fetching fresh data in background for key: ${CACHE_KEY}`
        );
        fetchFreshQuestions(activeOnly)
          .then((freshData) => {
            const dataToCache = {
      ...
      console.log(
        `[Cache Miss] No data found in local cache for key: ${CACHE_KEY}. Fetching new data...`
      );
    }
      ...
      themes.map(async (theme) => {
        try {
          ...
          if (!countResponse.ok) {
            ...
            console.warn(
              `Failed to fetch question count for theme ID ${theme.id}: ${countResponse.status}`
            );
...
const fetchUsers = async () => {
  ...
  try {
    ...
    users.forEach((user) => {
      console.log(
        `User ${user.first_name} ${user.last_name} (ID: ${user.id}, Role: ${user.userRoleId}):`
      );
      if (user.ip_addresses) {
        console.log(`  IP Addresses (${user.ip_addresses.length}):`);
        user.ip_addresses.forEach((ip) => {
          console.log(
            `    - ${ip.ip_address} (used ${ip.usage_count} times, last: ${ip.last_used})`
          );
          if (ip.is_banned) {
    ...
    const finalError = new Error(
      `Failed to fetch users from ${url}. Original error: ${error.message}`
    );
    throw finalError;
  }
};
...
const displayUserIpInfo = (user) => {
  console.log(
    `displayUserIpInfo called for user ${user.first_name} ${user.last_name} (ID: ${user.id})`
  );
  ...
  const ipInfo = user.ip_addresses
    .map((ip) => {
      let status = '';
      if (ip.is_banned) {
        status = ' [BANNED]';
...
const updateQuestion = async (question) => {
  try {
    ...
    const currentAnswers = Array.from(answerElements)
      .map((answerEl) => {
    ...
    const updateData = {
      ...
      no_answer_correct:
        noCorrectCheckbox?.checked !== undefined ? noCorrectCheckbox.checked : false,
    ...
    showNotification(
      `Question updated successfully! ${result.updated_answers || 0} answers updated.`,
      'success'
    );
...
const deleteItem = async (itemType, itemId) => {
  try {
    ...
    if (error.message.includes('403')) {
      ...
      showNotification(
        `${itemType.charAt(0).toUpperCase() + itemType.slice(1, -1)} not found. It may have been already deleted.`,
        'error'
      );
...
const showEditModal = async (itemType, item = null) => {
  ...
  if (itemType === 'questions') {
    ...
    if (state.themes && state.themes.length > 0) {
      themesOptionsHTML = state.themes
        .map((theme) => {
          // Compare theme IDs for proper selection
          const isSelected =
            item && ((item.theme && item.theme.id === theme.id) || item.themeId === theme.id);
      ...
      if (e.target.classList.contains('js-answer-count')) {
        ...
        if (answerListContainer) {
          ...
          const currentAnswers = Array.from(
            answerListContainer.querySelectorAll('.c-answer-item')
          ).map((answerItem) => {
...
<div class="c-form-group">
    ...
    <select name="ip" class="c-form-select">
        ...
        `
                ...
                .join('')
            : '<option value="">No IP addresses found</option>'
        }
    </select>
</div>
<div class="c-form-group">
...
<div class="c-form-group">
    ...
    window.banIpAddress = async function () {
      ...
      try {
        ...
        // Success
        showNotification(
          `IP address ${banData.ip_address} has been banned successfully!`,
          'success'
        );
...
const loadQuestions = async () => {
  ...
  try {
    ...
    const selectedThemeId = domAdmin.themeFilter?.value
      ? parseInt(domAdmin.themeFilter.value)
      : null;
    const selectedDifficulty = domAdmin.difficultyFilter?.value
      ? parseInt(domAdmin.difficultyFilter.value)
      : null;
    ...
    // Filter by theme
    if (selectedThemeId) {
      filteredQuestions = filteredQuestions.filter((q) => q.themeId === selectedThemeId);
    }

    // Filter by difficulty
    if (selectedDifficulty) {
      filteredQuestions = filteredQuestions.filter(
        (q) => q.difficultyLevelId === selectedDifficulty
      );
    }
    ...
    questionList.innerHTML =
      '<div class="c-error-state">Failed to load questions. Please try again.</div>';
  }
};
...
const createQuestionElement = (question) => {
  ...
  const themesOptions = state.themes
    .map((theme) => {
      const isSelected =
        question.themeId === theme.id || (question.theme && question.theme.id === theme.id);
...
<select id="answer-count-${question.id}" class="js-answer-count" data-question-id="${question.id}">
    ${Array.from({ length: 20 }, (_, i) => i + 4)
      .map(
        (num) => `<option value="${num}" ${answerCount === num ? 'selected' : ''}>${num}</option>`
      )
      .join('')}
</select>
            </div>
...
const loadThemes = async () => {
  ...
  try {
    ...
    if (themes.length === 0) {
      themeList.innerHTML =
        '<div class="c-empty-state">No themes found. Add a new theme to get started.</div>';
      return;
    }
    ...
    themeList.querySelectorAll('.js-delete-theme').forEach((btn) => {
      btn.addEventListener('click', async () => {
        ...
        if (theme && theme.questionCount > 0) {
          showNotification(
            `Cannot delete theme "${theme.name}" because it contains questions.`,
            'error'
          );
          return;
        }
    ...
    themeList.innerHTML =
      '<div class="c-error-state">Failed to load themes. Please try again.</div>';
  }
};
...
const loadUsers = async () => {
  ...
  try {
    ...
    if (!users || users.length === 0) {
      // Check if users is null/undefined or empty
    ...
    userTableBody.innerHTML = users
      .map((user) => {
        ...
        const lastActiveDisplay = user.last_active
...
const saveCurrentItem = async (itemType) => {
  ...
  if (itemType === 'questions') {
    ...
    try {
      ...
      if (!userId) {
        throw new Error(
          'User credentials not found. Please access this page with proper authentication.'
        );
      }
    ...
    try {
      ...
      const url = isEditing
        ? `${lanIP}/api/v1/themes/${currentEditItem.id}`
        : `${lanIP}/api/v1/themes`;
  ...
  // Basic validation
  const requiredFields = Object.entries(currentEditItem).filter(
    ([key]) => key !== 'id' && key !== 'lastActive' && key !== 'questionCount'
  );
...
const populateMigrationDropdowns = () => {
  ...
  state.themes.forEach((theme) => {
    const sourceOption = new Option(
      `${theme.name} (${theme.questionCount || 0} questions)`,
      theme.id
    );
...
const migrateQuestionsToTheme = async (sourceThemeId, targetThemeId) => {
  try {
    ...
    const response = await fetch(
      `${lanIP}/api/v1/themes/${sourceThemeId}/migrate-to/${targetThemeId}`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    showNotification(
      `Successfully migrated ${result.migrated_count} questions from "${result.source_theme.name}" to "${result.target_theme.name}"`,
      'success'
    );
  ...
  try {
    ...
    if (!stories || stories.length === 0) {
      ...
      if (createBtn) {
        createBtn.addEventListener('click', () => {
          ...
          overlay
            .querySelectorAll('.js-close-create-story')
          ...
          overlay.querySelector('.js-save-create-story').addEventListener('click', async () => {
            ...
            if (!name) {
              showNotification('Story name is required', 'error');
              return;
            }
            try {
              ...
              closeModal();
              showNotification(
                result.created ? 'Story created' : 'Story already existed',
                'success'
              );
              await loadStoriesTab();
            } catch (e) {
    ...
    container.querySelectorAll('.js-edit-story').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        const closeModal = () => overlay.remove();
        overlay
          .querySelectorAll('.js-close-edit-story')
          .forEach((b) => b.addEventListener('click', closeModal));
        overlay.addEventListener('click', (ev) => {
          if (ev.target === overlay) closeModal();
        });
        ...
        overlay.querySelector('.js-save-edit-story').addEventListener('click', async () => {
          ...
          if (!newName) {
            showNotification('Story name cannot be empty', 'error');
            return;
          }
          try {
            const userId = sessionStorage.getItem('admin_user_id');
    ...
    container.querySelectorAll('.js-view-story').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        const storyName = storyCard
          ? storyCard.querySelector('.c-story-card__title')?.textContent
          : 'Story';
        ...
        container
          .querySelectorAll('.c-story-card')
          .forEach((c) => c.classList.remove('c-story-card--selected'));
        ...
        try {
          const res = await fetch(
            `${lanIP}/api/v1/articles/by-story/${encodeURIComponent(storyId)}/`
          );
          if (!res.ok) throw new Error(`HTTP ${res.status}`);
          ...
          const articlesHtml = list
            .map((a) => {
              ...
              const intro =
                contentData.intro || a.story || a.excerpt || 'No introduction available';
          ...
          target.innerHTML =
            '<div class="c-error-state">Failed to load articles for this story.</div>';
        }
      });
    });
  } catch (err) {
