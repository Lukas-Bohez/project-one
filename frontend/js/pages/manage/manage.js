  ...
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('statPendingRequests').textContent =
        data.pending_time_off_requests || 0;
...
function applyEmployeeFilters() {
  ...
  const filtered = employees.filter((emp) => {
    ...
    const haystack =
      `${emp.first_name} ${emp.last_name} ${emp.email} ${emp.role_name || ''}`.toLowerCase();
  ...
  employeeList.innerHTML = list
    .map((emp) => {
      ...
      const role = escapeHTML(
        emp.position_title || emp.role_name || roleMap[emp.role_id] || 'Employee'
      );
...
function populateRoleSelect(selectedId) {
  ...
  roleSelect.innerHTML = employeeRoles
    .map((role) => {
...
async function handleEmployeeSubmit(e) {
  ...
  const basePayload = {
    ...
    hourly_rate: document.getElementById('employeeHourlyRate').value
      ? parseFloat(document.getElementById('employeeHourlyRate').value)
      : null,
