...
const demoData = {
  ...
  currentEmployee: { id: 2, name: 'Jamie Lee', role: 'Barista', hourly_rate: 16.0 },
};
...
function initializeDemoShifts() {
  ...
  demoData.business.employees.forEach((emp, empIndex) => {
    ...
    const daysWorking =
      empIndex === 4
        ? [1, 2, 3, 4, 5] // Manager works weekdays
        : empIndex % 2 === 0
          ? [0, 2, 4, 6]
          : [1, 3, 5]; // Alternating schedules
        ...
        <div class="demo-employee-portal">
            ...
            <div class="demo-employee-header">
                <div class="demo-employee-info">
                    <div class="demo-avatar">${employee.name
                      .split(' ')
                      .map((n) => n[0])
                      .join('')}</div>
                    <div>
        ...
        <div class="demo-request-card">
            <div class="demo-request-header">
                <div class="demo-request-employee">
                    <div class="demo-avatar-sm">${request.employee_name
                      .split(' ')
                      .map((n) => n[0])
                      .join('')}</div>
                    <div>
...
function renderTeamList() {
  return demoData.business.employees
    .map((emp) => {
      ...
      return `
            <div class="demo-team-card">
                <div class="demo-team-info">
                    <div class="demo-avatar" style="background: ${emp.color}">${emp.name
                      .split(' ')
                      .map((n) => n[0])
                      .join('')}</div>
                    <div style="flex: 1;">
...
function updateSubtaskInDOM(taskId, subtaskId, subtask) {
  const subtaskEl = document.querySelector(
    `.ui-subtask-card[data-task-id="${taskId}"][data-subtask-id="${subtaskId}"]`
  );
  if (!subtaskEl) return;
...
function showNewTaskModal() {
  ...
  const formHTML = window.ManageUI.tasks.renderTaskForm(employeeOptions, 'handleTaskFormSubmit');

  window.ManageUI.modal.show('Create New Task', formHTML, 'medium');
}
...
function createNewTask(taskData) {
  ...
  const newTask = new ManageShared.models.Task({
    id:
      demoData.business.tasks.length > 0
        ? Math.max(...demoData.business.tasks.map((t) => t.id)) + 1
        : 1,
...
function handleAddShift(event) {
  ...
  const newShift = {
    id:
      demoData.business.shifts.length > 0
        ? Math.max(...demoData.business.shifts.map((s) => s.id)) + 1
        : 1,
...
function updateTeamMember(event, employeeId) {
  ...
  const usernameTaken = demoState.employeeCredentials.some(
    (c) => c.username === newUsername && c.id !== employeeId
  );
  if (usernameTaken) {
    if (window.ManageUI) {
      window.ManageUI.notification.show('Username already exists! Please choose another.', 'error');
...
function employeeLogin(event) {
  ...
  const credentials = demoState.employeeCredentials.find(
