  ...
  try {
    ...
    const files = svgLinks
      ...
      .filter((filename) => {
        ...
        return lower.endsWith('.svg') && filename !== '' && !filename.includes('..'); // Security: no parent directory access
      });
...
const findMatchingSVG = (itemName, availableSVGs) => {
  ...
  matchingSVG = availableSVGs.find((svg) => {
    ...
    return (
      svgNameWithoutExt === itemNameLower ||
      svgNameWithoutExt === normalizedItemName.replace(/\s+/g, '-')
    );
  });
  ...
  if (!matchingSVG) {
    ...
    matchingSVG = availableSVGs.find((svg) => {
      ...
      return itemWords.some((itemWord) =>
        svgWords.some((svgWord) => itemWord.includes(svgWord) || svgWord.includes(itemWord))
      );
    });
  }
...
const setupItemClickHandler = (slotElement, item) => {
  ...
  const doDiscard = () => {
    console.log(
      `🗑️ Double-click detected for ${itemName} (${itemId}) - discarding item without activating`
    );
    // discard animation
...
const useItem = async (itemId, itemName, discard = false) => {
  ...
  try {
    const url =
      `/api/player/${currentUser.id}/items/${itemId}/use` + (discard ? '?discard=true' : '');
