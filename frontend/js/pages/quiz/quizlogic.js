...
class QuizSocketHandler {
  ...
  handleAllUsersDataUpdate(data) {
    ...
    if (currentUserId) {
      ...
      if (currentUserData) {
        this.questionHandler.currentUser = {
          ...this.questionHandler.currentUser,
          ...currentUserData,
        };
  ...
  handlePlayerUpdate(playerData) {
    ...
    const playerIndex = this.questionHandler.players.findIndex(
      (p) => p.user_id === playerData.user_id
    );

    if (playerIndex !== -1) {
      this.questionHandler.players[playerIndex] = {
        ...this.questionHandler.players[playerIndex],
        ...playerData,
      };
    } else {
