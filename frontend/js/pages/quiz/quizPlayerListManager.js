...
class PlayersListManager {
  ...
  getPlayerScore(player) {
    return player.session_score || player.score || player.points || player.total_score || 0;
  }
  ...
  isCurrentUser(player) {
    ...
    return currentUserId && playerUserId && String(currentUserId) === String(playerUserId);
  }
  ...
  generatePlayerHTML(player, index) {
    ...
    const questionsAnswered =
      player.total_questions_answered || player.questions_answered || player.answered_count || 0;
  ...
  updateSessionInfo(data) {
    ...
    const playerCountElement =
      document.querySelector('.player-count') || document.querySelector('#player-count');
