...
class AuthSystem {
  ...
  async autoLogin() {
    ...
    const looksLikePlaceholder =
      String(firstName || '').toLowerCase() === 'gamepad' &&
    ...
    const modalHTML = `
            <div id="authModal" class="c-modal" style="display: block;">
                <div class="c-modal__content">
                    <h2 id="authModalTitle">Join <a href="/" class="c-header__link gamepad">Quiz The Spire</a> Session</h2>
  ...
  showError(message) {
    const errorContainer =
      document.getElementById('authError') || document.getElementById('errorContainer');
    if (errorContainer) {
  ...
  shouldReloadPage() {
    ...
    const quizContainer = document.querySelector(
      '.c-quiz-container, #quiz-container, [data-quiz-container]'
    );
    if (!quizContainer) {
