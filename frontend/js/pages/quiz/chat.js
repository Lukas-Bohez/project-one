...
class ChatSystem {
  constructor() {
    ...
    this.fetchActiveSessionId().then(() => {
      if (typeof io !== 'undefined') {
        ...
        console.warn(
          'ChatSystem: Socket.IO not available. Chat will work without real-time updates.'
        );
        this.socket = null;
      }
      this.init();
      ...
      if (this.sessionId) {
        ...
        this.socket.emit('join', `quiz_session_${this.sessionId}`, (response) => {
          if (response && response.status === 'success') {
            ...
            console.error(
              `ChatSystem: Failed to join room: quiz_session_${this.sessionId}`,
              response
            );
          }
        });
    ...
    try {
      ...
      if (this.explicitSessionId) {
        if (activeSessionIdsList.includes(this.explicitSessionId)) {
          ...
          console.log(
            `ChatSystem: Waiting for explicit session ${this.explicitSessionId} to become active`
          );
        }
      ...
      if (!newestSessionId) {
        console.warn(
          'ChatSystem: No active sessions found. Waiting for a new session to be created.'
        );
  ...
  getUserFromLocalStorage() {
    try {
      ...
      if (userId && firstName && lastName) {
        ...
        console.log(
          'chatdebugconsolelog: Successfully created user from localStorage:',
          userFromStorage
        );
  ...
  getCurrentUserWithFallback() {
    ...
    if (userFromStorage) {
      console.log(
        'chatdebugconsolelog: Using user data from localStorage as fallback:',
        userFromStorage
      );
      return userFromStorage;
    }
    ...
    console.log(
      'chatdebugconsolelog: No user found in currentUser or localStorage, returning null'
    );
    return null;
  }
  ...
  async loadChatMessages() {
    if (!this.sessionId) {
      console.warn(
        'ChatSystem: loadChatMessages: Session ID is not yet available. Skipping chat load.'
      );
      return;
    }
  ...
  async retryPendingMessage() {
    ...
    console.log(
      `ChatSystem: Retrying pending message (attempt ${this.retryAttempts + 1}/${this.maxRetryAttempts})`
    );
    ...
    try {
      ...
      if (this.retryAttempts >= this.maxRetryAttempts) {
        ...
        this.showChatError(
          'Failed to send message after multiple attempts. Please try again manually.'
        );
        this.pendingMessage = null;
        this.retryAttempts = 0;
        this.stopSessionIdUpdater();
  ...
  async sendChatMessage() {
    ...
    console.log(
      'chatdebugconsolelog: All checks passed, sending message with user:',
      user,
      'sessionId:',
      this.sessionId
    );
  ...
  isScrolledToBottom() {
    ...
    const threshold = 50;
    return (
      chatMessages.scrollTop >= chatMessages.scrollHeight - chatMessages.clientHeight - threshold
    );
  }
...
document.addEventListener('userAuthenticated', (event) => {
  ...
  if (window.chatSystemInstance) {
    ...
    console.log(
      'chatdebugconsolelog: chatSystemInstance.currentUser set to:',
      window.chatSystemInstance.currentUser
    );
