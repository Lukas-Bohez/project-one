...
(function () {
  ...
  async function fetchFreshArticlesByStory(storyId) {
    const response = await fetch(
      `${CONFIG.apiBaseUrl}/articles/by-story/${encodeURIComponent(storyId)}/`
    );
    if (!response.ok) {
      ...
      if (!isValid(cached, CACHE_DURATION)) {
        fetchFreshArticlesByStory(storyId)
          .then((freshData) => {
            setCache(CACHE_KEY, freshData);
            console.log(
              `[Content Cache Update] Articles updated in background for story ${storyId}`
            );
  ...
  function buildNavigationHTML(articleIndex, totalArticles) {
    ...
    contentState.stories.forEach((story) => {
      const selected =
        contentState.currentStory && contentState.currentStory.id === story.id ? 'selected' : '';
    ...
    contentState.currentArticles.forEach((a, i) => {
      const num =
        a && a.id != null ? a.id : Number.isFinite(a?.story_order) ? a.story_order + 1 : i + 1;
