...
class SentleGame {
  ...
  async init() {
    ...
    if (this.sessionToken && this.username) {
      ...
      if (this.stats.playedToday) {
        ...
        this.showMessage(
          'You already played today. Come back tomorrow or view your stats/archives.',
          'info'
        );
        this.hideGameplayUI();
        this.loadLeaderboard();
  ...
  async startPracticeGame() {
    ...
    this.wordOrder = this.generateWordOrder(
      this.originalWords.length,
      this.practicePayload.wordOrder
    );
    ...
    if (!this.practiceMode && this.gameSessionId) {
      ...
      if (this.currentGuess === this.currentWord) {
        ...
        if (this.wordGuesses.length >= this.maxAttemptsPerWord) {
          ...
          this.showMessage(
            `Out of attempts for "${this.currentWord}". Moving to next word.`,
            'warning'
          );
  ...
  updateProgress() {
    ...
    if (progressText)
      progressText.textContent = `Word ${this.currentWordIndex + 1} of ${this.words.length}`;
  ...
  async loadLeaderboard(targetDate = null) {
    ...
    try {
      ...
      if (globalBoard)
        globalBoard.innerHTML = '<div class="loading">Unable to load leaderboard.</div>';
    }
    ...
    try {
      ...
      const dateText = targetDate
        ? `Unable to load scores for ${targetDate}.`
        : "Unable to load today's scores.";
  ...
  async revealLetter(letterIndex) {
    ...
    if (!this.practiceMode && this.gameSessionId) {
      ...
      this.revealsUsed++;
      this.showMessage(
        `Letter revealed! ${this.maxReveals - this.revealsUsed} reveals left`,
        'info'
      );
    }
  ...
  hasLocalPlayedToday() {
    ...
    try {
      ...
      if (!saved.userId && saved.username && this.username && saved.username !== this.username)
        return false;
  ...
  checkGameState() {
    ...
    this.originalWords =
      state.originalWords && state.originalWords.length > 0
        ? state.originalWords
        : this.sentence
          ? this.sentence.split(' ')
          : [];
    this.wordOrder =
      state.wordOrder && state.wordOrder.length > 0
        ? state.wordOrder
        : this.generateWordOrder(this.originalWords.length);
    this.words = this.wordOrder.map((idx) => this.originalWords[idx]);
