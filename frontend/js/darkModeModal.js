...
(function () {
  ...
  function createUI() {
    ...
    const pop = el(
      ...
      [
        ...
        el('div', { class: 'dm-popover__actions' }, [
          ...
          el(
            'button',
            { class: 'c-btn c-btn--sm c-btn--tertiary', type: 'button', id: 'dm-dark' },
            'Dark'
          ),
    ...
    fab.addEventListener('click', (e) => {
      ...
      fab.setAttribute('aria-expanded', pop.classList.contains('is-open'));
    });
    document.addEventListener('click', (e) => {
      if (!pop.contains(e.target) && e.target !== fab) close();
    });

    const ensureTM = () =>
      window.themeManager || window.themeManager?.applyTheme || window.setTheme;
    const setLight = () =>
      window.setTheme
    ...
    document.getElementById('dm-light').addEventListener('click', () => {
      ensureTM();
      setLight();
    });
    document.getElementById('dm-dark').addEventListener('click', () => {
      ensureTM();
      setDark();
    });
