...
function initializeArticleSelector() {
  ...
  // Automatically load the third article (index 2) on page load for SEO
  if (articles.length >= 3) {
    select.value = 2; // Set dropdown to third article
    loadArticle(2); // Load the third article
  }
}

// Load and display the selected article
function loadArticle(index) {
  ...
  const prevButton =
    prevIndex >= 0
      ? `<button class="nav-button prev-button" onclick="loadArticle(${prevIndex})">
            ...
            Previous: ${articles[prevIndex].title}
        </button>`
      : '';

  const nextButton =
    nextIndex < articles.length
      ? `<button class="nav-button next-button" onclick="loadArticle(${nextIndex})">
            Next: ${articles[nextIndex].title}
            <span class="button-icon">→</span>
        </button>`
      : '';
  ...
  // Update dropdown to match current article
  document.getElementById('article-select').value = index;
}
