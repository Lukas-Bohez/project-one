...
# Rebuild LofiExtension.zip from the lofi-extension folder
# Usage:
#   ./scripts/update_lofi_zip.sh                # use repo paths
#   ./scripts/update_lofi_zip.sh /path/to/src /path/to/downloads/out.zip

# Resolve directories relative to the script location (works when invoked from anywhere)
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
DEFAULT_SRC="$ROOT_DIR/lofi-extension"
DEFAULT_OUT_DIR="$ROOT_DIR/frontend/downloads"
DEFAULT_OUT="$DEFAULT_OUT_DIR/LofiExtension.zip"
...
TMP_BASE="$OUT_DIR/LofiExtension"
...
out_base = os.path.join(out_dir, 'LofiExtension')
...
CREATED_ZIP="$OUT_DIR/LofiExtension.zip"
